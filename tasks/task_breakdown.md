...
| ID   | タスク                                          | 内容                                                                                                                                                                        | 依存       | 対応                                                               |
| ---- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------------------------------------------------------------ |
| P3-1 | Workbench Shell                                 | Activity Bar、Primary/Secondary Side Bar、Panel、Status Bar、全体文字サイズ設定、各外周パネルおよび抽出／中間／チャンク／Resource編集内ペインの境界ドラッグリサイズと縦横スクロール、Secondary Side Barの縦アコーディオン、Editor Group/タブの左右・上下再帰分割、分割比率変更、タブDnD／Command移動、最大幅省略と多段表示、上部ボタン／Commandによる外周パネル表示切替、Explorer未確定要素数Badge、Activity並べ替え・Settings下端固定・選択表示、レイアウト保存・復元 | P0-2       | [UI-005, UI-006, UI-021, UI-022, UI-025, UI-026, UI-037〜043, sdd_ui_design §3, §9〜12] |
...
| P3-7 | Pipeline Navigator・Perspective                 | ①〜④ステージナビゲーション、作業モード（M1〜M5）切替、各ステージ一覧Editor（①原本、②抽出、③フェーズ－成果物、④モデル）への遷移、現在ステージだけの選択表示、列ソート、行のクリック／上下キー／Enter／Space操作、①～③の一覧／プレビュー境界リサイズ                                                                                                              | P3-2       | [UI-046〜047, sdd_ui_design §3.1, §5, §6]                                       |
...
| P4-2 | 原本ビュー     | 取込済み原本のソート可能な一覧と読取専用詳細、Windows関連付けアプリでのファイル表示、一覧からの論理削除・アーカイブ／復元、Explorer ①〜④見出しの強調・折りたたみ、各データ行のプロパティTooltip                                                                                                                                                                     | P3-2, P4-1       | [UI-010, UI-044〜047]                   |
...
| P5-15 | 抽出データビュー                 | ②抽出データのソート可能な一覧・独自プレビュー・論理削除・アーカイブ／復元、子要素からの文書単位ステータス集約、原本ファイル名を初期名称とする表示名称変更、一覧／プレビュー2ペインの境界リサイズ、文書プレビュー／色付き階層 structure_json 表示切替（形式非依存の共通ビュー）                                                                                                                                                                                                                                                                                                                                                                                 | P3-4, P5-2        | [UI-011, UI-046〜047, EXT-040〜041, UI-044〜045]                                                                   |
...
| ID   | タスク                           | 内容                                                                                                                                                                                                  | 依存       | 対応                                                                   |
| ---- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------------------------------------------------------------------- |
| P7-1 | 中間文書生成・統合               | プロジェクト設定画面での開発フェーズ配下の成果物管理、有効化・無効化・関連③を含む物理削除、Explorerの③見出し「取込」から取込先成果物1件（未選択可）と取込元②複数件を選択し、既存関係を初期選択として復元する統合元選択、複数②抽出文書の成果物単位統合、章構成・親子関係・参照関係・段落関係の保持、文書単位およびintermediate_item→extracted_itemのbased_on trace_link自動同期、フェーズ間成果物トレース | P5-6, P2-1 | [MID-001〜003, DATA-002, DATA-009, DATA-018, sdd_data_structure §2.11] |
...
| P7-7 | 中間データビュー・統合編集       | ③開発フェーズ－成果物のソート可能な一覧と独自プレビュー、子要素からの文書単位ステータス集約、「中間データ取込編集画面」の統合元extracted_item／intermediate_item／構造プレビュー3分割と「中間データ単独編集画面」のintermediate_item／構造プレビュー2分割（各境界をドラッグでリサイズ）、文書プレビュー／色付き階層 structure_json 表示切替、Ctrl/Shift複数選択、統合元の上下キー選択移動、extracted_item↔intermediate_itemの多対多based_on・青字取込済み表示・件数表示・個別紐付解除、上下追加・上下統合、階層編集、連続選択移動、状態サイクル、要素レビュー、成果物とプレビューのresource種別ラベル、図表表示、3ペイン対応強調、独立縦横スクロール、Properties同期、マージ/分割、要約作成（LLM）、設計モデルへの反映導線                                  | P3-6, P7-2 | [UI-012, UI-046, MID-007, EDIT-002〜006, EDIT-008, LLM-035]                             |
...
| P8-6 | 設計モデルビュー          | ④設計要素・関係のソート可能な一覧・詳細ビュー                                                                                                                                                                                            | P3-4, P8-1       | [UI-013, UI-046]                                                                         |
