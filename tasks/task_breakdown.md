...
| P7-2 | 中間データ ID・編集・マージ/分割 | 各情報単位への ID 付与、取込編集／単独編集の画面切替、単独編集の成果物＋プレビュー2ペイン、任意位置追加・Resource複製・削除・item_type表示・14 resource種別固有カラム編集・種別変更時の情報消失確認（ラベル／ダブルクリック／Space／Enter起動）、resource://共通Editor Provider、人間によるマージ・分割と新 ID 割当・由来追跡、抽出誤り修正の反映                                                                                                        | P7-1, P5-3 | [MID-004, MID-005, EXT-024]                                            |
