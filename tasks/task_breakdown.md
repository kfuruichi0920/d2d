...
| P7-3 | 設計観点別管理 | sdd_data_structure 4.6のresource_* 14種の固有カラム保持・定義駆動編集、全Resourceの管理用特記事項、全フィールド説明、テキスト／ラベル／リスト／図／表／数式／コードの編集定義更新、図・表・数式・疑似コードの説明LLM候補と派生Resource関係 | P7-1 | [MID-002, MID-010〜012, MID-020〜024, EDIT-076〜086] |
...
| P8-5 | 候補採用トランザクション  | 候補セット採用時の同一トランザクション検査（許容関係・重複・未解決参照・根拠リンク不足）、一時 ID→UUIDv7 変換、entity_registry + resource_* + trace_link への正本反映、設計要素→生成元チャンクのbased_on、採用/修正/棄却履歴の保存                              | P8-4, P8-2       | [MODEL-006, MODEL-009, LLM-039, sdd_data_structure §2.10]                        |
