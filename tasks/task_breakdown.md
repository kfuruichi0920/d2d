...
| P8-4 | 候補セットレビュー Editor | `candidate://` Resource、保存前の表形式追加・修正・削除・種別変更、分類接頭辞＋全体連番の一時ID、要素名・分類変更時の関係 From/To 追従、タブ間移動中のメモリ保持、原本と別の一時保存1セット／再開、オントロジー許容マトリクスで許可された関係だけの選択肢と許容外LLM初期値の警告、正規化テキスト・要素候補・関係候補・警告・LLM ログ・検証エラーの同期表示                                   | P3-8, P8-3       | [MODEL-007, MODEL-008, MODEL-030, MODEL-031, UI-035, UI-036, LLM-038]                                  |
...
| P8-7 | 設計モデル設定・Help | Settings配下の独立ページで設定方法を示し、モデル／属性／関係の日本語定義、属性のGUI追加・無効化・削除、モデル／関係の有効化・無効化・追加、関係別2次元許容マトリクス、版確定を管理し、Helpを保存済み定義から動的生成 | P8-1, P8-2, P2-2 | [MODEL-019〜029, UI-060〜061] |
