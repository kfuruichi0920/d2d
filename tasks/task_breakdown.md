...
| P5-7  | Excel 抽出 | `extract.excel`: OOXMLを直接読み、セル・結合セル・行列構造・シート名位置を抽出する。候補レビュー以降はP5-19を正とする | P5-1, P5-2 | [IMP-002, EXT-009] |
...
| P5-10 | PDF 抽出ワーカー                 | `extract.pdf` / `extract.pdf.region`: ページ画像・ページ寸法・テキスト（座標・フォント・色・太字/斜体）・線・矩形・画像・リンク・表（bbox・二次元配列・ヘッダー候補）・領域候補（見出し/本文/リスト/表/図/キャプション/ヘッダ/フッタ/ページ番号/装飾）。テキスト・表抽出は pdfplumber、ページ画像レンダリング・bbox クロップは pypdfium2 を採用（TBD-01 改定 2026-07-22: AGPL 依存なし、商用版差替え不要）。詳細は `tasks/P5-20-pdf-extraction.md` を正とする | P5-1, P5-2, P2-2  | [IMP-005, EXT-012, EXT-027, sdd_data_structure §2.9]                                            |
| P5-11 | PDF 抽出レビュー Editor          | ページ画像上の領域枠の移動・リサイズ・追加・削除・種別変更・採否・読み順（ズーム対応）、表エディタと領域単位の再解析、警告表示。P5-20B として実装。詳細は `tasks/P5-20-pdf-extraction.md` を正とする                                                                                                                                                                                                          | P3-8, P5-10       | [EXT-028, EXT-029, EXT-031〜033, UI-031, UI-032]                                                |
| P5-12 | PDF LLM OCR・補正                | 選択領域単位の Vision モデルによる OCR・表 OCR・テキスト補正・数式 LaTeX 化候補生成 → 人間レビュー後反映（専用 OCR エンジン不使用）。P5-20D として実装                                                                                                                                                                                                                                                        | P5-11, P6-1〜P6-5 | [EXT-030]                                                                                       |
...
| P5-19 | Excel抽出候補フロー | `extract.excel`: `.xlsx` の物理抽出、ルール候補、シートプレビュー上の編集可能オーバレイ、任意範囲LLMグルーピング、DrawingML図・装飾、再取込差分・候補UID継承、確定候補から既存②抽出データへの変換を実装する。生成／確認操作分離、2値採否、②生成時の全候補差分基準保存、Excel互換キー移動と安全な範囲選択を含む。詳細は `tasks/P5-19-excel-extraction.md` を正とする | P5-7, P5-6, P6 | [IMP-002, EXT-009, EXT-049〜067, sdd_excel_extraction] |
...
| P9-4 | 汎用トレースマトリクスEditor             | 設計分類／②文書／③成果物／チャンク／Resource種別から複数Resource集合を行列へ配置し、設定済みの有効・無効全関係種別の個別／全選択ON/OFF、設定駆動アイコン、必須属性入力、複数関係種別の色分け表示、転置、固定見出し、ズーム、Tooltip、十字強調、行／列／セル複数選択、方向指定による単一トグル・「選択」「削除」での一括追加／論理削除、Activity Bar起動時の全設計モデル×全設計モデル初期化、固有URIでの複数タブ表示を提供する | P9-1, P3-4, P8-2 | [UI-014, TRACE-026〜029, TRACE-038, TRACE-040〜043]                      |
| P9-5 | 汎用インパクト分析ビュー                 | Pipeline分析起動時は②全抽出・③全中間・チャンク全件・全設計モデルの4列で初期化し、チャンクを含む任意の複数Resource集合を持つ列を左右へ追加・DnD並替えし、見出しドラッグによる列間隔調整と外側列の連動移動、列別スクロール、全列組合せの方向・関係種別リンク、リンク表示ON/OFF、クリック／上下キー／Shift複数選択とSecondary連携、全列への連鎖強調、関連項目限定表示、階層折畳と親リンク集約、Tooltip、表示領域限定リンク描画、プロジェクト別の名前付き構成保存・復元、固有URIでの複数タブ表示を提供する | P9-1, P3-4 | [UI-015, TRACE-030〜040]                      |
