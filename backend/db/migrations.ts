...
export const MIGRATIONS: Migration[] = [
  ...
  {
    version: '2.5.0',
    description: 'PDF抽出領域候補ドラフトを追加（P5-20 / IMP-005、EXT-027〜028）',
    apply(db) {
      db.exec(`
        CREATE TABLE IF NOT EXISTS pdf_extraction_draft (
          source_document_uid TEXT PRIMARY KEY,
          status TEXT NOT NULL DEFAULT 'generated'
            CHECK (status IN ('generated', 'editing', 'confirmed', 'failed')),
          physical_json TEXT NOT NULL
        ...
        CREATE INDEX IF NOT EXISTS idx_pdf_extraction_draft_status
          ON pdf_extraction_draft(status, updated_at);
      `)
    }
  }
]
