...
| P5-19A〜C                  | Excel .xlsx のOOXML物理抽出、抽出グループ候補レビュー、選択範囲限定LLM支援、既存②抽出データへの確定変換（schema 2.3.0、Unit 304／E2E 33）                                                                                                                                                                                    | （本コミット）  |
...
- スキーマ変更は `backend/db/migrations.ts` に追記（バックアップ → DDL → 版数更新）。現在 2.3.0（2.2.0の13個の `model_*`、`model_beh`、関係の作成中状態・設定駆動アイコン、LLM候補一時保存に加え、`excel_extraction_draft` を追加。2.2以前からの後方互換なし、旧PJは再作成）。
- ②抽出レビューの選択・状態更新・構造プレビュー・Properties は `ReviewElement` 共通契約で実装し、
  Word 固有にしない。今後の Excel / PowerPoint / PDF / Visio / テキスト系も同じ操作体系へ接続する。
- **Excel抽出は候補確定まで②正本と分離する**。`extract.excel` はマクロを実行せずOOXMLを直接読み、物理モデルとルール候補を `excel_extraction_draft` に保持する。LLMへ送るのはユーザーが選択した候補範囲と周辺2セルだけで、未選択シート／候補を含めない。確定時だけ採用候補を `storeExtractionResult` 経由で既存②へ変換する。図形・画像・コネクタはP5-19D、再取込差分・UID継承はP5-19E。
...
- P5 他形式抽出: PowerPoint / PDF / Visio / テキスト系（P5-8〜14）。Excelの後続はP5-19D（DrawingML図形・画像・コネクタ）とP5-19E（再取込差分・範囲プロファイル再利用・UID継承候補）
...
- LLMログからの候補再作成は design-candidates（④候補生成）のみ。intermediate.textCandidate 等の他処理種別・
  プロンプト編集付き再実行は未対応。トースト自動消去は info（5秒）を E2E で検証済み、error（15秒）は時間都合で未検証（実装は notify の setTimeout で共通）。
