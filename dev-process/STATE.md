...
- テスト規模: ユニット 257 件 / pytest 16 件 / E2E 27 件（すべて成功の状態で引き渡し）
...
- UI点検（2026-07-19）の未対応項目: Secondary見出し・Activity名の英語表記（Properties等、付録A仕様のため据置）、
  Editorタブのタイトル命名規則（Job/Git差分等のID露出）、タブ多段折返し時の整理（グループ化・スクロール化）、
  SecondaryのDictionary検索入力の意匠統一、Explorer情報行（「編集する場合は…」）の配置見直し。
