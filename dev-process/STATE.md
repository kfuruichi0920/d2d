...
- 現在の改修: schema 2.1.0。LLM候補のタブ間編集保持・1セット一時保存／再開・分類別一時IDと採用時正式採番、④モデル一覧の明示編集／日時列／右クリック関係登録、Settings配下の独立した設計モデル設定とGUI属性編集を反映した。2026-07-20 に正規ゲート `node dev-process/verify.mjs --e2e`（typecheck / lint / format / ユニット270件 / build / E2E 28件）を完走し、ALL PASSを確認済み。
...
- スキーマ変更は `backend/db/migrations.ts` に追記（バックアップ → DDL → 版数更新）。現在 2.1.0（13個の `model_*`、オントロジー設定、LLM候補一時保存。1.xからの後方互換なし、旧PJは再作成）。
