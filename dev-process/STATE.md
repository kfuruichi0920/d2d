...
| P5-20C                     | PDF確定→②変換: pdf.confirmジョブ（採用・非除外領域→読み順で②要素へ、図はcrop切出し→blob、caption_of連結、source_locationへpage+bbox保持）、確定後の読取専用再表示、store-extractionのページ座標拡張（Unit 315／E2E 34）                                                                                                      | （本コミット）  |
