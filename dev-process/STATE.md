...
| P5-20A                     | PDF物理抽出・領域候補ドラフト: extract.pdf／extract.pdf.region（pdfplumber+pypdfium2、AGPL排除・TBD-01改定）、schema 2.5.0 `pdf_extraction_draft`、ルール候補（見出し/本文/リスト/表+セル/図/キャプション/ヘッダ等既定不採用）、読み順推定、pdfDraft API 4種（Unit 312／pytest 30／E2E 33）                                  | 6fb2c18         |
| P5-20B                     | PDFオーバーレイレビューEditor: pdf-draft://、ページ画像+候補オーバーレイ、ズーム/ページ切替、ドラッグ追加・移動・リサイズ、種別/採否/読み順/bbox編集、表セル編集+領域再解析、図CSS切出しプレビュー、一括採否、原本/ステージ導線（Unit 312／E2E 34）                                                                          | ef9c4cb         |
| P5-20C                     | PDF確定→②変換: pdf.confirmジョブ（採用・非除外領域→読み順で②要素へ、図はcrop切出し→blob、caption_of連結、source_locationへpage+bbox保持）、確定後の読取専用再表示、store-extractionのページ座標拡張（Unit 315／E2E 34）                                                                                                      | 0d90077         |
| P5-20D                     | PDF LLM支援: 選択領域＋近傍ブロック限定のLLM分類（ブロック境界スナップ・pdf-region-refinement）、Vision OCR（領域切出しPNG添付・文字/表/数式TeX・候補保存→ユーザー適用でmanual_text/table_data反映・pdf-region-ocr）、送信前確認・外部送信ブロック経由、SRSマーク更新（Unit 317／E2E 34）                                    | 9a22767         |
...
- **PDF抽出は候補確定まで②正本と分離する**（P5-20、検討資料「13-Pdf抽出方針」）。`extract.pdf` は
  pdfplumber(MIT)+pypdfium2(Apache/BSD) で物理抽出とページPNG生成を行い、AGPLのpymupdfは使用しない
  （TBD-01改定済み。商用版でのPDFライブラリ差替えは不要）。物理モデルと領域候補は `pdf_extraction_draft`
  に保持し、座標はPDFポイント（左上原点）を正、表示は `image_scale`×zoom で換算する。候補UIDはBackendが
  UUIDv7採番し、ワーカー内キー（candidate_key/caption_of_key）は保存時にUIDへ差替える。header/footer/
  page_number/decoration は既定 `review_status='rejected'`（非破壊除外）で、確定時に②へ変換しない。
  領域単位の再解析は `extract.pdf.region`（table/text/crop）で行い、文書全体は再解析しない。
  LLM支援は候補生成に限定し自動確定しない: 分類LLMへは選択領域＋近傍60ptのブロックだけを送り、
  提案範囲は suggested_block_ids のブロック境界へスナップする（任意座標を採用しない）。Vision OCRへは
  領域切出しPNGだけを添付し、結果は `llm_suggestion.ocr` に保存、ユーザー適用で `manual_text` /
  `table_data` へ反映する。`manual_text` は保存時の物理テキスト再計算と②変換より常に優先される。
- **PDF抽出はワーカー初のサードパーティ実行時依存**（workers/python/requirements.txt: pdfplumber、
  pypdfium2）。実行するPythonへ `python -m pip install -r workers/python/requirements.txt` が必要。
  未導入環境では extract.pdf が明確なエラーを返す（Word/Excel抽出は従来どおり標準ライブラリのみで動く）。
...
- PDF抽出のE2E・実機動作には、Backendが起動するPython（PATHの `python` または環境変数 `D2D_PYTHON`）に
  pdfplumber / pypdfium2 が必要。この開発機はPATH先頭のvenvにpipが無いため、
  `D2D_PYTHON=C:\Users\kfuru\miniconda3\python.exe` を設定して `npx playwright test` /
  `node dev-process/verify.mjs --e2e` を実行する（miniconda側は導入済み）。E2Eフィクスチャ生成
  （tests/make_pdf.py）は標準ライブラリのみで、どのPythonでも動く。
- E2Eのスキーマ版数表示検証（`title-schema-version`）はマイグレーション追加のたびに更新が必要
  （app.spec.ts の `schema 2.x.0` 文言。2.5.0追加時に1件失敗した実例あり）。
- ②抽出レビューの `element-grid` 一覧は各要素の text／タイトルを表示し、表のセル内容は行の本文表示側にしか
  出ない。E2Eでセル内容やページ座標トレースを検証する場合は `extracted.get` の `elements`
  （page_no_start / bbox / rows を含む）を使う（PDF E2Eで 'Controller' 直接参照が失敗した実例あり）。
