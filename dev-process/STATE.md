...
- テスト規模: ユニット 220 件 / pytest 10 件 / E2E 25 件（すべて成功の状態で引き渡し）
...
| P3/P11追加    | 上部メニューバーの旧ボタン意匠・戻る／進む／更新／ホーム・マウス履歴・可変アドレス・お気に入り、タブ移動Command、Search内グループTree・文書配下本文検索／要素ジャンプ（Unit 226／E2E 25） | 本コミット      |
...
- 上部メニューバーは戻る／進む／更新／ホーム、件数付きの①～④、抽出▶／統合▶／モデル化▶、分析、用語集、可変幅Resourceアドレスバー、画面内検索、お気に入りを固定順で表示する。①～④・分析・用語集は境界と背景を持つ同一ボタン意匠とし、左側ボタンを縮小せずアドレス入力だけを画面幅へ追従させる。戻る／進むはAlt+左右矢印とマウスbutton 3/4を同じResource履歴へ接続し、更新はアクティブEditorを再読込、ホームはダッシュボードを開く。お気に入りはURIと変更可能な表示名をプロジェクト単位でlocalStorageへ保存しExplorerへ表示する。
...
- Search Activityの検索結果は検索ボタン直下へResource種別ごとの折畳Treeで表示し、10件以下を初期展開、10件超を初期折畳とする。上下キーは選択移動とEditorプレビュー、左右キーはグループ折畳／展開へ割り当てる。原本／抽出文書／中間文書の検索種別は配下itemのResource本文もFTS対象とし、結果は対象文書を開いてitem UIDまたはresource UIDで該当要素を選択・スクロールする。下段PanelにSearch Resultsタブは置かない。
