...
- 現在の改修: schema 2.0.0 オントロジー再設計を反映済み。2026-07-20 に正規ゲート `node dev-process/verify.mjs --e2e`（typecheck / lint / format / ユニット270件 / build / E2E 28件）と Python pytest 16件を完走した。全検証で検出したテーマ設定の非同期保存競合、`model_*` 関係先の `design://` 遷移欠落、`model_state` 専用状態遷移エディタの統合欠落を修正済み。
...
- スキーマ変更は `backend/db/migrations.ts` に追記（バックアップ → DDL → 版数更新）。現在 2.0.0（13個の `model_*` とオントロジー設定。1.xからの後方互換なし、旧PJは再作成）。
