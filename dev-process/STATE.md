...
- テスト規模: ユニット 140 件 / pytest 10 件 / E2E 16 件（すべて成功の状態で引き渡し）
...
- 統合済み抽出要素は③リソースから `based_on` 祖先を辿って判定する。派生要素が全削除された場合のみ再統合可能。
...
- ③成果物の状態サイクルは `draft → approved → review → rejected → draft`。成果物ペインの上下矢印はスクロールではなく選択アイテム移動に割り当てる。
