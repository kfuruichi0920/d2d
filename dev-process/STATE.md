...
- テスト規模: ユニット 262 件 / pytest 16 件 / E2E 27 件（すべて成功の状態で引き渡し）
...
| P3/P9追加            | インパクトSVG端点補正、Alt+G／F5、アクティブGroup表示、Status Bar実行環境状態（Unit 262／pytest 16／E2E 27）                                                                              | —               |
...
- Status BarのGit同期状態はネットワーク通信を行わず、ローカルupstream参照に対するahead／behindを表示する。PlantUML／MeCabの有効状態は、設定優先・同梱fallbackで解決した実体パスの存在により判定する。
...
- **docs/ の設計文書は実装準拠で維持する**（2026-07-20 整理済み）。各文書の冒頭に実装準拠状態の注記があり、
  ...
  該当文書のマークと本文を同時に更新する。スキーマの正本は `backend/db/migrations.ts`（sdd_data_structure に
