...
- **設計分析（ANA-001〜006）は読み取り専用・決定論的**。クエリ規則DSLは行ベース（`FROM TYPE` / `TRAVERSE 関係 UP|DOWN|BOTH DEPTH n` / `FILTER TYPE|STATUS` / `PATH 関係 MAXDEPTH n LIMIT m`、`#`コメント）で、保存・実行前に構文とオントロジー関係定義の照合を行う。スロット10種の正本はプロジェクト設定 `analysis.querySlots`（スキーマ変更なし）で、未設定プロジェクトは既定2件（影響範囲下流3段・経路検索）を返す。PATHは起点・終点必須、FROMなしTRAVERSEは起点必須。実行上限は集合2000要素・深さ10・経路200件で、超過は truncated を立てて打ち切る。レポートは `exports/reports/analysis_*.md` へ保存し既存の report:// プレビュー・Reports一覧へ相乗りする。
...
- MCPサーバの残り（将来実装）: プロジェクトフォルダだけで動く独立MCPサーバ化、SSEストリーム（サーバ通知）・セッション管理・認証、MCP検索のMeCabトークナイズ対応。インパクト分析・経路検索は設計分析（ANA）としてUI実装済みだが、MCPツールとしての公開は未対応。
- 設計分析の残り: アクセスログのローテーション（logs/mcp と同様にデバッグログ全般で未実装）、DSLの拡張（関係属性・要素属性値によるフィルタ、集合演算、否定条件）、分析結果のグラフ表示（現状はレポートのみ）、レポートのHTML形式出力。
