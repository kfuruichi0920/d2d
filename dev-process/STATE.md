...
| P5-20D                     | PDF LLM支援: 選択領域＋近傍ブロック限定のLLM分類（ブロック境界スナップ・pdf-region-refinement）、Vision OCR（領域切出しPNG添付・文字/表/数式TeX・候補保存→ユーザー適用でmanual_text/table_data反映・pdf-region-ocr）、送信前確認・外部送信ブロック経由、SRSマーク更新（Unit 317／E2E 34）                                    | （本コミット）  |
