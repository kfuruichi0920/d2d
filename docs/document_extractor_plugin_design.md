...
本ドキュメントは、文書抽出プラグインソケットに接続されるプラグインプラグの設計要求を定義する。

文書抽出プラグインソケットは、①原本データを入力し、原本ファイル単位の②抽出データを生成する。③中間データおよび④設計モデルは直接更新しない。

関連文書は以下である。

- [要求仕様書](srs_main.md)
- [プラグイン詳細設計書](plugin_detail_design.md)
- [文書抽出プラグイン 共通設計要求](extractor_common_design_requirements.md)
- [Word文書情報抽出プラグイン 詳細要求](word_extractor_requirements.md)
- [Excel文書情報抽出プラグイン 詳細要求](excel_extractor_requirements.md)
- [PowerPoint文書情報抽出プラグイン 詳細要求](powerpoint_extractor_requirements.md)
- [PDF文書情報抽出プラグイン 詳細要求](pdf_extractor_requirements.md)
- [Visio文書情報抽出プラグイン 詳細要求](visio_extractor_requirements.md)

## 2. 対象プラグインソケット

| 項目 | 内容 |
| --- | --- |
| プラグインソケット種別 | 文書抽出プラグインソケット |
| 主入力 | ①原本データ、取込設定、抽出設定 |
| 主出力 | ②抽出データ成果物フォルダ |
| 成果物単位 | 原本ファイル単位 |
...
| 責務 | Word、Excel、PowerPoint、Visio、PDF、テキスト、Markdown、CSV、TSV、JSON等の形式別抽出プラグインプラグを接続する |
...
| 連携先 | 用途 |
| --- | --- |
| LLMプロバイダプラグインソケット | OCR補正、表候補、図表説明候補、意味分類候補の生成 |
| 履歴管理プラグインソケット | 抽出前後、再抽出前後、ZIPアーカイブとの差分確認 |
| トレーサビリティプラグインソケット | ②抽出データの情報単位IDに対する関係確認 |
| 中間データ処理プラグインソケット | ②抽出データを入力として③中間データを生成する後続処理 |
