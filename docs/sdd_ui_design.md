...
| Pipeline Navigator | データ階層の変換順序と分析・用語集・Resource URIの入口 | ①〜④を変換名付きで表示し、現在開いている1ステージだけを選択表示する。分析、用語集、編集可能なアドレスバーからEditorを開く |
...
| 項目 | 仕様 |
| --- | --- |
| 表示順 | `①原本-(抽出)->②抽出-(統合)->③中間-(モデル化)->④モデル　分析　用語集　アドレスバー` の固定順で表示する |
...
| アドレスバー | アクティブEditorの内部Resource URIを表示して直接編集を受け付ける。Enter時に既知URIとして検証し、不正な場合はエラー通知して画面遷移しない |
