...
| Settings | 「ツール設定」と「プロジェクト設定」を分離。ツール設定はテーマ、PlantUML、検索、LLM Provider、APIキー、新規プロジェクトのGit初期化可否（既定有効）を管理し、プロジェクト設定は成果物、開発フェーズ、LLM外部送信可否を管理する。プロジェクトを開いている場合は同じSettings Editor内にプロジェクトDBを正本とするオントロジー定義と許容マトリクスを表示する | `settings.open`、`projectSettings.open` |
...
オントロジー設定は、プロジェクトを開いているとき既存のSettings Editor内に表示する。別Activityや別設定パネルは増やさない。

| 領域 | UI・動作 |
| --- | --- |
| バージョン | 現在版（初期 `0.1.0`）を表示し、「確定」でパッチ版を更新する |
| モデル定義 | `model_*` ごとに日本語名、層、日本語定義、独自項目定義、有効状態を編集する。削除操作は設けない |
