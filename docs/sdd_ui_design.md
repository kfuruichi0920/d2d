...
共通Resource Editorの text / multiline 欄は `SemanticTextInput` を利用する。入力欄は編集、プレビュー、構造化データを切り替え、`Ctrl+Space` または入力候補ボタンで前方一致候補を開く。候補は最近使用、辞書用語、モデル要素をグループ表示し、最小文字数未満では大量候補を描画しない。既存文解析は登録済み用語・同義語・モデル名の文字範囲、正規化差分、未登録用語を候補として表示し、個別の承認・取消し後にだけ保存対象とする。
