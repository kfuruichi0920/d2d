...
| パーツ | 役割 | D2D での用途 |
| --- | --- | --- |
| Title Bar / Command Center | 全体状態とコマンド入口 | 左端はD2Dだけを表示し、中央にコマンドパレット入口、右端にPrimary／Secondary／Panelのアイコンのみの表示切替を配置する |
...
中間データ取込編集画面と中間データ単独編集画面では、文書構成を規定する`structure_json.elements`をインデント付き構成アウトラインとして表示する。アウトライン、成果物一覧、中間文書プレビューは同一`intermediate_item`選択を共有し、プレビューまたはアウトラインから選択した行が仮想スクロール範囲外の場合は成果物一覧を当該行へスクロールする。中間データ取込編集画面と中間データ単独編集画面の成果物一覧、および中間文書プレビューは、抽出時の `element.type` ではなく `intermediate_item.item_type` をResource種別ラベルとして表示する。ラベルのクリック、成果物行のダブルクリック、選択行でのSpace／Enterは、すべて同じ共通Resource Editorを開く。成果物一覧の操作バーにはResource固有操作を置かず、LLM支援・正規化・種別固有編集は共通Resource Editor内に集約する。
