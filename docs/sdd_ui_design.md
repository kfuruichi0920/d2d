...
- Excel原本（.xlsx）の抽出操作は、候補未生成時に「抽出グループ候補を生成」、候補生成済み／編集中は「抽出グループ候補を確認」を表示する。候補Editor上部はシートドロップダウンとシート別採否件数、左は省略しない疎セルプレビュー、キーボード／Shift／マウス範囲選択、種別別の編集可能候補オーバレイ、右は選択シートだけの色付きタグ付きコンパクト一覧、複数選択、右クリック一括採否、縦リサイズ可能な詳細とする。詳細は範囲文字列、図プレビュー、表タイトル行・列設定を表示し、任意矩形だけをLLMグルーピングへ送れる。「抽出データを生成」で採用候補を既存②抽出データへ変換して抽出レビューEditorを開く。候補確定前は②一覧へ表示せず、確定後は再抽出をUI/API双方で拒否する。
