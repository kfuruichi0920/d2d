...
- Excel原本（.xlsx）の抽出操作は、候補未生成時に「抽出グループ候補を生成」、候補生成済み／編集中は「抽出グループ候補を確認」を表示する。候補Editorはシートタブ、セルグリッド、矩形候補一覧、範囲・種別・タイトル・採否編集を同一画面で提供し、選択候補だけをLLM支援の送信前確認へ渡す。「抽出データを生成」で採用候補を既存②抽出データへ変換して抽出レビューEditorを開く。候補確定前は②一覧へ表示せず、確定後は再抽出をUI/API双方で拒否する。
