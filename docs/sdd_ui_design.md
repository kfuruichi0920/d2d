...
ツール設定は共通トークンのユーザ上書きとして、Workbench背景、パネル・サーフェス、文字、補助文字、境界線、アクセント、選択背景、ボタン背景・文字・境界を個別に保存する。上書き未設定のトークンは表示モードとカラーテーマの既定値を使用し、個別または一括で既定へ戻せる（UI-052）。
...
| 項目 | 仕様 |
| --- | --- |
| Git操作・履歴 | 状態一覧で複数ファイルをステージ／解除し、ローカルブランチを作成／切替する。コミット時はDB to Text＋SQLite dumpの再生成結果を必ず含める。コミット一覧から特定時点のDB to Textを参照し現行Diffを開く（GIT-001〜007） |
...
| History | Git状態・ステージ・コミット・ローカルブランチ・履歴、DB to Text、ZIP差分比較対象 | `git.stage`、`git.unstage`、`git.commit`、`git.branchCreate`、`git.checkout`、`diff.open`、`history.openCommit` |
...
共通Resource Editorの text / multiline 欄は `SemanticTextInput` を利用する。通常時は背景色を他の入力部品と分けた表示文章のプレビューと編集ボタンだけを表示する。編集ボタン、またはフォーカス中のプレビューで `Enter`／`F2` を押すとモーダルな編集ダイアログを開き、入力、プレビュー、構造化データ、既存文解析、入力候補、LLM用語候補、構造化参照、解析差分、入力欄設定をダイアログ内へ集約する。編集中の `Ctrl+Space` または入力候補ボタンで前方一致候補を開く。候補は最近使用、辞書用語、モデル要素をグループ表示し、最小文字数未満では大量候補を描画しない。既存文解析は登録済み用語・同義語・モデル名の文字範囲、正規化差分、未登録用語を候補として表示し、個別の承認・取消し後にだけ保存対象とする。
