...
| ステージクリック | 対応する作業モード(5章)へ切り替え、Editor Areaにステージ一覧を開く。Pipeline Navigatorの選択色は作業モードではなく現在の `stage://` Resourceで判定し、①～④のうち1件だけを選択表示する。①原本は取込操作＋取込済み原本一覧＋読取専用詳細（ファイル表示はWindows関連付けアプリへ委譲）、②抽出は抽出文書一覧＋D2Dプレビュー＋EXPLORE選択案内、③中間は取込操作＋開発フェーズ－成果物階層一覧＋D2Dプレビュー、④設計モデルは種別・ID・名称・状態・作成日時・更新日時の一覧と `+設計モデル`／`+PlantUML` を表示する。`+設計モデル` は一覧へ行を追加するだけとし、状態遷移・用語集ボタンは表示しない。各列見出しは昇順／降順ソートを切り替える |
| 分析 | 汎用インパクト分析を開き、初期構成をリスト1=②全抽出データ、リスト2=③全中間データ、リスト3=チャンク全件、リスト4=全設計モデルとする |
...
| 一覧操作 | 全ステージ一覧は選択行を薄青背景で表示し、上下矢印キーで選択行とフォーカスを移動し、Enter／Spaceはクリックと同じ動作を行う。④モデルはシングルクリックで選択し、ダブルクリックまたはEnterで個別編集画面を開き、右クリックで他モデルとの関係登録ダイアログを開く。①～③の一覧／プレビュー境界はドラッグまたは矢印キーで変更する。①原本、②抽出、③中間では `status='deleted'` の論理削除とアーカイブ／復元を提供し、アーカイブはデータを保持したままExplorerだけから除外する |
...
| `settings://` | `settings://tool` | Tool Settings Editor |
| `settings://design-model` | `settings://design-model` | Design Model Settings Editor |
...
| `help://design-model` | 現在のオントロジーバージョン、設定済み `model_*` の層・日本語定義・有効状態、関係種別と日本語定義をDBから動的に表示する |
...
| 反映 | 採用時に④正本（`entity_registry` + 対応する `model_*`）を作成し、各採用モデルから候補生成を要求したチャンクへ `trace_link`（`based_on`、`llm_run_uid` つき）を張る |
| 追跡 | 候補セットから LLM ログ(プロンプト、応答、token、コスト)へ1クリックで辿れる(LLM-012、LLM-015) |
| 一時保存 | 原本候補とは別にDBへ編集途中1セットを保存する。再度開いた直後は原本を表示し、「一時保存を再開」で保存内容へ切り替える。採用成功時は一時保存を削除する |
...
| 候補レビュー | 候補セットをグリッドで点検し、根拠チャンク・③本文と対照して採用 / 修正 / 棄却する。保存前に要素候補と関係候補を追加、修正、削除でき、要素名・分類変更時は同じ候補セット内の一時IDと関係候補From/Toを追従表示する。関係の必須属性は任意入力とし、未入力で採用した関係は仮値と「作成中」で保存する。編集後も再検証して採用操作を有効に保つ |
...
トレースマトリクスは設計分類固定の参照表ではなく、台帳登録されたResource集合間の`trace_link`を俯瞰確認・編集するWorkbench Resourceとする。Activity Barの「新しいトレースマトリクス」から開く場合は、行軸・列軸をともに「④ 全設計モデル」で初期化する。行軸・列軸はそれぞれ、設計分類、②抽出文書配下の`extracted_item`、③中間文書配下の`intermediate_item`、チャンク、Resource種別を複数選択して和集合を構成する。③中間文書は正本Resourceを複数束ねる表示スコープであり、文書自体をセル要素へ展開しない。

| 項目 | 仕様 |
| --- | --- |
| 関係表示 | `ontology_relation_definition` の全定義を有効・無効を問わず表示し、個別選択と全選択ON/OFFを提供する。関係別の `icon_color` と `icon_text` を選択肢とセルへ共通使用し、無効な定義を明示する。表示対象の関係を1件以上持つ行・列見出しを強調する |
| 方向 | 【未適用】編集方向は「行→列」「列→行」を上部で指定するが、保存時はrelation_typeごとの正規方向へ変換し、許可表と一致しない方向を拒否する。セル内には実際のfrom/to方向を矢印で示す。行列転置時は表示軸だけを交換し、保存済みfrom/toは変更しない。`conflicts_with` は1件を双方向表示する |
| 単一編集 | 修飾キーなしのセルクリックは、選択中の関係種別について設定方向の関係をトグルする。存在しない関係は `ontology_relation_definition` / `ontology_relation_allowance` による型・方向・必須属性・重複・階層制約検査後に追加し、存在する関係は関係台帳の `entity_registry.status='deleted'` へ論理削除する。関係種別に `required_attr` がある場合は同じ関係選択領域で値を入力でき、未入力時は仮値を補って `review_status='creating'`（作成中）とする |
| 複数編集 | Ctrl/CmdまたはShift付きセルクリック、行見出し、列見出しで対象セル集合を選択する。上部の「選択」「削除」は全対象を同一トランザクションで追加／削除し、1件でも検証エラーなら全体を反映しない |
| 選択表示 | アクティブセルの行・列全体を十字に強調し、複数選択セルは別の選択背景で示す |
| 大表対応 | 表領域だけをスクロールし、列見出しと行見出しはsticky表示する。表示倍率は60〜160%で変更できる |
| Tooltip | 行・列見出しはcode、title、entity_type（model_type）、status、所属スコープを表示する |

マトリクス取得APIは表示スコープ一覧、行列Resource、両方向の関係リンクを返す。編集APIは対象セル対、関係種別配列、関係種別ごとの必須属性値、方向、操作（add/delete/toggle）を受ける操作単位APIとし、Rendererからセル単位の細粒度DB操作を反復しない。


候補セットグリッドは、要素候補と関係候補を別タブまたは上下分割で表示する。関係候補の必須属性は同じグリッドで任意入力し、未入力採用時は仮値と作成中状態を付与する。候補一時IDは分類接頭辞と候補セット全体の連番（`req-01`、`struct-02`等）とし、要素候補の分類変更時は一時IDと関係候補のFrom/Toを即時追従させる。採用時は現在のDB採番状態から正式code・uidを割り当て、一時IDを正本へ保存しない。タブ間移動中の編集はRendererメモリへ保持し、タブを閉じるかF5で破棄する。明示的な一時保存は原本と別に1セットだけDBへ保持し、再度開いた直後は原本を表示して「一時保存を再開」で復元する。未解決参照、許容外関係、重複、根拠なし候補はValidation Resultsへ出し、該当行を選択すると候補グリッド、③根拠範囲、LLMログが同期する。
...
| 項目 | 仕様 |
| --- | --- |
| リンク表示 | 【未適用】表示中の全列組合せについて`trace_link`を曲線で結び、複数ステップ離れた列間も省略しない。from→to方向を小型矢印、`relation_type`を色・短縮記号で識別する。10種類から複数を同時選択でき、`conflicts_with` は保存方向にかかわらず双方向到達可能として表示する。描画ON/OFFを切り替え、項目・リンクのhoverで台帳プロパティをTooltip表示する |
...
| Settings | 「ツール設定」と「プロジェクト設定」を分離。ツール設定はテーマ、PlantUML、検索、LLM Provider、APIキー、新規プロジェクトのGit初期化可否（既定有効）を管理し、プロジェクト設定は成果物、開発フェーズ、LLM外部送信可否を管理する。Settings Primary Side Barは「ツール設定を開く」「└ 設計モデル設定」「プロジェクト設定を開く」の順とし、設計モデル設定は独立ページでプロジェクトDBを正本とする定義・属性・許容マトリクスを表示する | `settings.open`、`designModelSettings.open`、`projectSettings.open` |
...
設計モデル設定はSettings Activity配下の独立Resource `settings://design-model` として表示する。Primary Side Barでは「ツール設定を開く」と「プロジェクト設定を開く」の間に「└ 設計モデル設定」を置き、ツール設定の一部であることを示す。画面冒頭には定義・属性・許容マトリクス・版確定の設定手順を表示する。

| 領域 | UI・動作 |
| --- | --- |
| バージョン | 現在版（初期 `0.1.0`）を表示し、「確定」でパッチ版を更新する |
| モデル定義 | `model_*` ごとに日本語名、層、日本語定義、有効状態を編集する。モデル自体は削除せず無効化する |
| 属性定義 | 独自属性をGUIで追加し、既存属性を無効化または削除できる。無効属性は入力検証と個別モデルEditorの入力欄から除外する |
| モデル追加 | `model_` で始まる種別ID、code prefix、日本語定義、項目定義を入力し、対応する物理テーブルと定義を作成する |
| 関係定義 | relation_type、日本語名、日本語定義、必須属性、有効状態を編集・追加する。削除操作は設けない |
| 許容マトリクス | 選択した関係種別について、縦軸を起点 `model_*`、横軸を終点 `model_*` とし、許容セルをチェックで切り替える。無効モデルを識別可能に表示する |
| `based_on` | 終点が①〜③のためモデル間マトリクスを表示せず、その理由を説明する |
| 更新連携 | 保存後に `ontology.updated` を通知し、Help、Pipeline、候補レビュー、設計モデルツリー／Editorが採用中定義を再読込する |
| Help連携 | Help Editorは保存済みの版、モデル定義、関係定義、許容組合せを再読込して表示する |

個別設計モデルEditorはオントロジーの `field_schema_json` から入力部品を生成し、共通情報（タイトル、要約、状態）と種別固有情報を同じ画面で編集する。
## 10. Editor Area
...
| Settings Editor | `settings://tool` / `project-settings://current` | ツール設定、プロジェクト設定 |
| Design Model Settings Editor | `settings://design-model` | 設定方法、モデル／属性／関係定義、許容マトリクス、版確定 |
...
| Review | 【未適用】選択アイテムに対する任意コメントの入力・保存と既存コメント一覧。コメントはMGMT分類の`resource_text.text_role='comment'`として保存し、コメントを`from_uid`、選択アイテムを`to_uid`として、①〜③のSRC対象には`based_on(basis_kind='human_approved')`、④設計要素には`relates_to(status='provisional', reason='awaiting_review')`を同一トランザクションで作成する |
...
構造化データは `schemaVersion=1`、`originalText`、`displayText`、`policy`、`references[]` のJSONとして直接編集できる。検証成功まではフォームへ反映せず、Resource保存前にも再検証する。モデル参照の承認時は起点・終点分類に対して `ontology_relation_definition` / `ontology_relation_allowance` が許可する関係種別だけを選択できる。自動既定は `relates_to` とし、暫定理由の入力、強い意味関係の明示承認、型・固定方向・必須属性・重複検証を必須とする。LLMによる未登録用語抽出は既存の外部送信許可・マスキング・ジョブログを経由し、結果を辞書へ自動登録しない。
...
- Pipeline Navigatorは `①原本-(抽出)->②抽出-(統合)->③中間-(モデル化)->④モデル`、分析、用語集、Resourceアドレスバーを固定順で表示する。選択表示はactiveなステージURIだけを基準とし、①〜④を排他的に表示する。分析は全抽出／全中間／チャンク全件／全設計モデルの4列で汎用インパクト分析を開く。アドレスバーは既知URIだけを受理し、不正時は通知して遷移しない。`nav.focusAddress`（既定Alt+D）はアドレスバーへフォーカスして全選択し、`nav.refresh`（既定F5）はメニューバーの更新と同じくアクティブEditorを再読込する。`editor.group.activate.1`〜`.9`（既定Ctrl+1〜9）は分割ツリーを先頭から深さ優先で並べた画面順のEditor Groupをアクティブ化し、該当Groupがない場合は何もしない。作業モードCommandに既定キーは持たせない。すべてショートカットキー設定から変更・解除できる。Status Barには作業モード名・Resource URI・テーマ名を表示せず、プロジェクト名、ジョブ状態、Gitブランチ／upstream同期状態、PlantUML／MeCab利用可否、LLM Provider設定状態、外部送信可否、デバッグログレベルを意味別のSerendieアイコン付きで表示する。PlantUML／MeCab／Providerのクリックはツール設定、外部送信可否／ログレベルのクリックはプロジェクト設定を通常タブで開く。Git同期状態はネットワーク通信を行わず、ローカルに保持されたupstream参照に対するahead／behindとする。①〜④の一覧行は薄青背景で選択を示し、上下矢印で選択行を移動、Enter／Spaceでクリックと同じ操作を実行する。④モデルは単一クリックで選択し、ダブルクリックまたはEnterで開き、右クリックでモデル間関係登録ダイアログを開く。①〜③の一覧／プレビュー境界は共通 `ResizablePaneGroup` で変更する。
...
- Excel原本（.xlsx）の抽出操作は「抽出グループ候補を生成」と「抽出グループ候補を確認」を並べ、候補未生成時は確認を、生成済み時は生成を無効化する。確認は②生成後も `stage://source` の原本詳細から繰り返し開ける。候補Editor上部はシートドロップダウンとシート別の採用／不採用件数、左は省略しない疎セルプレビュー、表示追従する矢印／Shift／Excel互換Ctrl+矢印／保持範囲内へ制限したマウス範囲選択、種別別の編集可能候補オーバレイ、右は選択シートだけの色付きタグ付きコンパクト一覧、複数選択、右クリック一括採否、縦リサイズ可能な詳細とする。採否は採用／不採用の2値で初期値を採用、種別はtextを既定に明確な構造だけをtable／list／figure等とする。詳細の範囲文字列は右幅内で折り返して高さを直接変更でき、他入力は自然高を維持する。任意矩形だけをLLMグルーピングへ送れる。独立した保存ボタンは設けず、「抽出を実行して②を生成」で採用候補を既存②抽出データへ変換すると同時に不採用を含む全候補を次回差分基準として保存する。候補確定前は②一覧へ表示せず、確定後は②の重複生成をUI/API双方で拒否する。
