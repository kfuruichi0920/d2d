...
| 領域 | 仕様 |
| --- | --- |
| アウトライン | `intermediate_document.structure_json` の章節・親子関係・順序を編集する |
| 文書風 Editor | ③中間データを原本に近い文書として表示・編集する（`intermediate_item` → `resource_*` 参照） |
| 図表表示 | 図（`resource_figure`）、表（`resource_table`）、説明文（`resource_text`）を本文と対応付けて表示する |
| Secondary Side Bar | 対応する②抽出データ（`trace_link`）、LLM 候補（`llm_run_ref`）、レビュー記録（`entity_registry.review_info_json`）を表示する |

### 11.3 V-04 設計モデルビュー

| 領域 | 仕様 |
| --- | --- |
| 設計要素ツリー | `entity_registry.entity_type` で種別を分類し、`trace_link`（`relation_type='based_on'` 等）で根拠リンクを辿る |
| 関係一覧 | `trace_link` を表形式で表示し、`relation_type`、`direction`、`confidence`、レビュー状態を編集する |
| モデル Editor | PlantUML / SysMLv2 テキストと要素 ID 対応表（`resource_model`）を表示する |
| Secondary Side Bar | 根拠中間データ（`trace_link`）、関連要素、未接続、影響範囲を表示する |
...
| エッジ | `trace_link` の `relation_type` で関係種別（`based_on`、`satisfies`、`verifies` 等）を色分けして表示する |
...
| `sdd_data_structure.md` | `entity_registry`、`trace_link`、`llm_run_ref` を Evidence、Relations、Logs に表示する |
