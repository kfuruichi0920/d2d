...
`ref/support_srs_260607` のPoCで有効だった「入力文書、正規化結果、抽出エンティティ表、抽出関係表、グラフ、AI通信ログ」を、D2DではM3の単一ページではなくWorkbench Resourceとして分解する。左側に③中間データとチャンク、中央に候補セットグリッドと④モデル編集、右側にEvidence / Relations / Candidates / Review、下部PanelにLLM Logs / Validation Resultsを置く。PoCのカード最小化・最大化・リサイズの考え方は、Workbenchのタブ、分割ペイン、Panel表示切替、レイアウト復元に置き換える。
