...
| `help://design-model` | 【未適用】SRS 9章の13分類と層構造、文書世界と設計モデル世界の分離、10関係種別と許可表、モデル内部情報、候補と確定情報、人間レビュー、`owner_uid`と`allocated_to`の区別 |
...
| 項目 | 仕様 |
| --- | --- |
| 関係表示 | 【未適用】10種類の`relation_type`から複数を同時選択し、関係別の色・短縮記号をセルへ重ねて表示する。選択可能なrelation_typeは行列の設計分類に対する `relation_rule_master` の許可組合せへ絞り、表示対象の関係を1件以上持つ行・列見出しを強調する |
...
| 単一編集 | 【未適用】修飾キーなしのセルクリックは、選択中の関係種別について設定方向の関係をトグルする。存在しない関係は `relation_rule_master` による型・方向・必須属性・重複・階層制約検査後に追加し、存在する関係は関係台帳の `entity_registry.status='deleted'` へ論理削除する。`relates_to` の追加時は暫定理由を入力する |
...
【未適用】構造化データは `schemaVersion=1`、`originalText`、`displayText`、`policy`、`references[]` のJSONとして直接編集できる。検証成功まではフォームへ反映せず、Resource保存前にも再検証する。モデル参照の承認時は起点・終点分類に対して `relation_rule_master` が許可する関係種別だけを選択できる。自動既定は `relates_to` とし、暫定理由の入力、強い意味関係の明示承認、型・固定方向・必須属性・重複検証を必須とする。LLMによる未登録用語抽出は既存の外部送信許可・マスキング・ジョブログを経由し、結果を辞書へ自動登録しない。
