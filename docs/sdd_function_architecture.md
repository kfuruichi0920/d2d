...
| プロジェクト管理 | プロジェクトファイルを開き、利用するプロジェクトを切り替える | project_id、プロジェクトルート、設定、schema_versionの解決 |
...
| ストアアクセス管理 | JSON / JSONL、SQLite DB、画像、GraphDBまたは関係グラフ索引への安全なアクセスを扱う | 正本データの読込・保存、トランザクション境界、schema_version整合確認、索引更新の呼び出し |
...
本ツールの機能は、複数の個別機能から横断的に利用される共通機能と、4階層データの生成・編集を担当する個別機能に分ける。共通機能と個別機能はいずれも、基盤機能を経由して成果物、設計モデル、設定、ログへアクセスし、直接ファイルI/OやDB更新を行わない。

### 4.1 共通機能

共通機能は、複数の個別機能から横断的に利用される支援機能である。

| 共通機能 | 主責務 | 主な入力 | 主な出力 | 利用する基盤機能 | 主な利用元 |
| --- | --- | --- | --- | --- | --- |
| トレーサビリティ機能 | ②抽出データ、③中間データ、④設計モデルの情報単位IDに対して、根拠・由来・変換関係、関係表示、関係クエリ、影響分析を行う | `trace_subject`、`trace_link`、`design_relation`、②③④のID | 関係クエリ結果、表、階層リスト、グラフ、JSON / CSV / Markdown | プロジェクト管理、設定管理、ジョブ管理、成果物管理、ストアアクセス管理、イベント通知、必要に応じてLLM実行ログ管理 | 文書抽出機能、中間データ処理機能、設計編集機能、履歴・差分参照機能、レポート出力機能 |
| 履歴・差分参照機能 | Git履歴、DB to Text、SQLite dump、ZIP差分から変更前後を参照する | Git履歴、DB to Text、SQLite dump、ZIPアーカイブ、②③④成果物 | Diffビュー、差分結果、履歴参照ビュー | プロジェクト管理、設定管理、ジョブ管理、成果物管理、ストアアクセス管理、イベント通知 | 文書抽出機能、中間データ処理機能、設計編集機能、トレーサビリティ機能、レポート出力機能 |
| LLMプロバイダ機能 | LLMによる候補生成、要約、分類、関係候補生成を提供する | source-groundedな入力チャンク、プロンプト、プロジェクト設定 | 候補情報、LLM実行参照、プロンプトログ、応答ログ | プロジェクト管理、設定管理、ジョブ管理、LLM実行ログ管理、イベント通知 | 文書抽出機能、中間データ処理機能、設計編集機能、トレーサビリティ機能、レポート出力機能 |
| レポート出力機能 | ②抽出データ、③中間データ、④設計モデルから文書風レポート、一覧、関係情報を生成する | ②抽出データ、③中間データ、④設計モデル、トレース情報、フィルタ条件 | Markdown / HTML レポート | プロジェクト管理、設定管理、成果物管理、ストアアクセス管理、イベント通知、必要に応じてLLM実行ログ管理 | 文書抽出機能、中間データ処理機能、設計編集機能、トレーサビリティ機能、履歴・差分参照機能 |

### 4.2 個別機能



個別機能は、①原本データ、②抽出データ、③中間データ、④設計モデルの生成・編集を担当する主処理機能である。個別機能同士の連携は、原則として成果物、設計情報、トレース情報、イベントを介して行う。

| 個別機能 | 主責務 | 主な入力 | 主な出力 | 利用する基盤機能 | 連携する共通機能・個別機能 |
| --- | --- | --- | --- | --- | --- |
| 文書抽出機能 | ①原本データから、原本ファイル単位の②抽出データを生成する | ①原本データ、抽出設定 | ②抽出データ、抽出ログ、画像リソース | プロジェクト管理、設定管理、ジョブ管理、成果物管理、ストアアクセス管理、イベント通知、必要に応じてLLM実行ログ管理 | LLMプロバイダ機能、トレーサビリティ機能、履歴・差分参照機能、レポート出力機能 |
| 中間データ処理機能 | ②抽出データを成果物単位に統合し、③中間データを生成・整理する | ②抽出データ、既存③中間データ、成果物定義 | ③中間データ、チャンク、用語候補、正規化表、図表説明候補 | プロジェクト管理、設定管理、ジョブ管理、成果物管理、ストアアクセス管理、イベント通知、必要に応じてLLM実行ログ管理 | 設計編集機能、LLMプロバイダ機能、トレーサビリティ機能、履歴・差分参照機能、レポート出力機能 |
| 設計編集機能 | ③中間データおよび④設計モデルを対象に、設計内容の編集、検索、レビュー補助、モデル表現編集を行う | ③中間データ、④設計モデル、用語、レビュー記録 | 更新済み③中間データ、④設計要素、設計関係、PlantUML / SysMLv2テキスト、要素ID対応表 | プロジェクト管理、設定管理、成果物管理、ストアアクセス管理、イベント通知、必要に応じてLLM実行ログ管理 | LLMプロバイダ機能、トレーサビリティ機能、履歴・差分参照機能、レポート出力機能 |
...
| `source.imported` | 文書抽出機能 | 本ツール側基盤機能、UI | 原本取込完了通知 |
| `extraction.completed` | 文書抽出機能 | 本ツール側基盤機能、UI、トレーサビリティ機能、履歴・差分参照機能 | ②抽出データ生成通知 |
| `artifact.updated` | 本ツール側基盤機能 | UI、中間データ処理機能、トレーサビリティ機能、履歴・差分参照機能 | 成果物フォルダ更新通知、差分参照対象の更新通知 |
| `intermediate.updated` | 中間データ処理機能 | 本ツール側基盤機能、UI、トレーサビリティ機能、履歴・差分参照機能 | ③中間データ更新通知 |
...
外部ワーカー（Node.js / Python / Rust / Go 等）は、ジョブ管理からサブプロセスとして起動される。通信は stdin / stdout を用いた改行区切りJSON（JSONL形式）で行う。ワーカーはファイルシステムや DB に直接アクセスせず、基盤機能 API 経由で操作する。
...
| 正本更新 | ワーカーは成果物フォルダや SQLite DB を直接更新しない。出力を基盤機能（ストアアクセス管理）に渡して更新させる |
