...
本章のシーケンス図では、UIを Renderer、Electron Shell を Main、業務ロジックとストア操作を Local Backend として扱う。Renderer は表示範囲や操作要求を出し、Main は IPC / OS 統合 / Backend 管理を担当し、Local Backend が `project.db`、`blobs/`、解析、キャッシュ、表示用データ生成を実行する。データライフラインとして①原本データ、②抽出データ、③中間データ、④設計モデルを明示する。
