...
| プロジェクト管理 | プロジェクトを作成し、プロジェクトファイルを開いて利用対象を切り替える | project_uid、プロジェクトルート、標準フェーズ・成果物、設定、schema_versionの解決。新規作成時はツール設定に従ってGit初期化をbest effortで実行する |
| 設定管理 | アプリ全体設定とプロジェクト別設定を管理する | APIキー参照、モデル設定、パス、プロキシ、テーマ、ショートカット、外部送信可否、新規プロジェクトのGit初期化可否の参照。設定エクスポート時はAPIキー等の機密情報を除外する（SRS CORE-046/047） |
...
| --- | --- | --- | --- | --- |
| トレーサビリティ機能 | 文書世界と設計モデル世界を分離し、文書由来、設計意味、実装・検証の関係登録、型・方向・階層・重複・意味重複検査、暫定関係率、関係クエリ、影響分析を行う | `entity_registry`、`model_*`、`trace_link`、`ontology_model_definition`、`ontology_relation_definition`、`ontology_relation_allowance`、`llm_run_ref` | 検証結果、関係クエリ結果、表、階層リスト、表示用サブグラフ、JSON / CSV / Markdown | 必要に応じてLLM実行ログ管理 |
| Git操作・履歴差分機能 | Git状態、ステージ／解除、コミット、ローカルブランチ、履歴、DB to Text、SQLite dump、ZIP差分を扱う | Git作業ツリー、DB to Text、SQLite dump、ZIPアーカイブ、②③④成果物 | Git操作結果、Diffビュー、差分結果、履歴参照ビュー | コミット直前にDB to Text＋SQLite dumpを再生成・自動ステージ |
...
| 利用するAPI | UI と同じ基盤機能 API（プロジェクト管理、ジョブ管理、ストアアクセス管理、設定管理等）。Secondary Side Barは `secondary.listRelations`、`secondary.listReviews`、`secondary.addReview` と辞書候補用の `semantic.search`／`glossary.addTerm` を利用する |
| 状態連携 | 【未適用】Workbench共通Selection、Context、Eventを介して、Editor、Secondary Side Bar（Properties／Relations／Review／Dictionary）、Panel、Status Barを同期する。Review保存時はMGMT分類のコメントResourceを作成し、①〜③のSRC対象には`based_on`、④設計要素には暫定理由付き`relates_to`を同一トランザクションで作成して、更新EventでRelationsとReviewを再取得する |
...
| 設計意味関係 | ④設計モデル上の意味関係は、台帳登録された `model_*` 間の `trace_link` として扱う |
...
| ④設計モデル | 正本 | `entity_registry` と種別ごとの `model_*` 詳細テーブル、`trace_link` を中心に `project.db` へ保存し、大容量データは `blobs/` に分離する |
...
| 履歴・差分参照の横断利用 | Git操作・履歴差分機能は、Git状態・ステージ・コミット・ローカルブランチ操作を提供し、Git履歴、DB to Text、SQLite dump、ZIPアーカイブを用いて変更前、特定時点、時点間差分を参照する |
...
Rendererからの作成要求をLocal Backendのプロジェクト管理が処理し、project.dbとファイルレイアウトを生成した同じ作成処理内でCORE-013の標準5フェーズ・18成果物を登録する。ツール全体設定`project.initializeGitOnCreate`は未設定を`true`として扱い、有効時はプロジェクトファイル生成後に`git init`を実行する。Git未導入、権限不足、コマンド失敗はプロジェクト作成の失敗条件にせず、そのまま作成済みプロジェクトを開く。
...
sequenceDiagram
    ...
    Host->>Trace: entity_registry、resource_*、model_*、trace_linkの索引を確認
...
sequenceDiagram
    ...
    Main->>Host: 確定内容を渡す
    Host->>DesignModel: ④設計モデルと関係をentity_registry、model_*、trace_linkへ保存
...
| ontology.updated | オントロジー設定管理 | Renderer、LLM候補生成、Help、Pipeline、設計モデルEditor | モデル・関係定義、有効状態、許容組合せ、確定版の更新通知 |
| `semantic.updated` | セマンティック入力支援機能 | Renderer、Secondary Side Bar、トレーサビリティ機能 | 入力欄の表示文章・構造化参照・正規化履歴の更新通知 |
...
チャンク作成・更新は `chunk.uid → intermediate_item.uid` の `based_on` を構成項目ごとに同期する。LLM候補採用では候補生成ログの `input_ref_uid` から生成元チャンクを解決し、採用した④設計要素から当該チャンクへ `based_on (basis_kind=inferred, llm_run_uid=<run>)` を同一トランザクションで登録する。生成元チャンクが欠落または別中間文書所属の場合は採用しない。

| 設計対象 | 実装上の扱い | 方針 |
| --- | --- | --- |
| テキストの曖昧性排除・校正・正規化 | ③中間データの選択範囲またはチャンクから正規化テキスト候補を生成する | 正本本文を直接上書きせず、候補セットとしてレビューする |
| 要素候補と関係候補のJSON出力 | `llm_run_ref` の結果、候補セット、`entity_registry` / 対応 `model_*` / `trace_link` への採用候補に分ける | JSON Schema検証と許容関係検査を必須にする |
...
| 正本保存 | ストアアクセス管理が `entity_registry`、対応 `model_*`、`trace_link` を更新する | 専用の単純な要素・関係テーブルは作らず、D2D共通台帳とトレース関係へ統合する |
...
| プロンプト選択・編集・送信前確認 | UI Workbench / プロンプト管理 / LLMプロバイダ機能 | 各LLM実行画面で用途別テンプレートを選択・実行時編集・新版保存し、送信先、モデル、外部送信可否、マスキング後の全メッセージを確認する。明示承認後だけ確認済みメッセージとtemplate UIDをジョブ化する |
...
| 区分 | 内容 | 保存先 |
| --- | --- | --- |
| 文書メタデータ | title、creator、created、modified、last_modified_by 等 | `extracted_document.structure_json.metadata` |
...
| 区分 | 内容 | 保存先 |
| --- | --- | --- |
| 文書メタデータ | ファイル名、スライド数、スライド寸法、抽出器名、抽出器バージョン、原本ハッシュ | `extracted_document.structure_json.metadata` |
...
| 区分 | 内容 | 保存先 |
| --- | --- | --- |
| 文書メタデータ | ファイル名、ページ数、ページ寸法、抽出器名、抽出器バージョン、原本ハッシュ | `extracted_document.structure_json.metadata` |
...
Resource説明生成は図・表・数式・疑似コードを対象とし、Resource値とアウトライン文脈をJSON本文として送る。図はプロジェクトルート内の `image_uri` だけを読み込み、OpenAI/Azureでは `image_url` data URL、Geminiでは `inlineData`、Ollamaでは `images` 配列へ変換する。画像バイナリは送信前確認でMIME種別を表示し、既存の外部送信許可・マスキング・実行ログを経由する。応答は `{"description":"..."}` を検証して保存前編集欄へ反映する。
...
| `DATA-010〜011` | ④設計モデルを `entity_registry`、`model_*`、`trace_link` として `project.db` に保存し、関係探索には `trace_link` 由来の関係グラフ索引を利用可能とする。現行実装では SQLite の再帰 CTE で走査。GraphDB は将来オプション。 |
| `DATA-020〜024` | DB to Textを②③④共通の派生成果物または一時成果物として扱う |
| `DATA-030〜033` | ZIP生成時のみmanifestを作成し、差分比較用インポートを履歴・差分参照機能で扱う |
| `sdd_data_structure.md` の `entity_registry` / `resource_*` / `model_*` | ①〜④の共通台帳、②③Resource、④設計モデルの詳細情報として扱う |
| `sdd_data_structure.md` の `trace_link` | 台帳登録されたリソース間の文書由来、設計意味、実装・検証関係として扱う。relation_type は10種類、方向はrelation_typeごとに固定し、モデル・関係の日本語定義と許容マトリクスはオントロジー設定テーブルを正規メタモデルとして共通利用する |
...
| `EDIT-057〜073` | 設計編集機能配下の共通セマンティック入力支援、通常プレビュー／編集ダイアログ、用語集、LLMプロバイダ、トレーサビリティ、固定順Secondary Dictionaryの協調として対応 |
