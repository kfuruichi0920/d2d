...
| 状態連携 | Workbench共通Selection、Context、Eventを介して、Editor、Secondary Side Bar（Properties／Relations／Review／Dictionary）、Panel、Status Barを同期する。Review保存時はコメントResourceと対象への`relates_to`を同一トランザクションで作成し、更新EventでRelationsとReviewを再取得する |
