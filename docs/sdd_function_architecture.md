...
| --- | --- | --- | --- | --- |
| トレーサビリティ機能 | 【未適用】文書世界と設計モデル世界を分離し、文書由来、設計意味、実装・検証の関係登録、型・方向・階層・重複・意味重複検査、暫定関係率、関係クエリ、影響分析を行う | `entity_registry`、`resource_*`、`trace_link`、`relation_rule_master`、`llm_run_ref` | 検証結果、関係クエリ結果、表、階層リスト、表示用サブグラフ、JSON / CSV / Markdown | 必要に応じてLLM実行ログ管理 |
...
| 採用前検査 | 【未適用】トレーサビリティ機能 / ストアアクセス管理 | 正規メタモデル `relation_rule_master` により型・固定方向・必須属性・重複・自己包含・循環・意味重複・未解決参照・SRC根拠リンク・レビュー状態を検査する |
...
| 関係候補 | 【未適用】relation_type候補は、文書由来専用 `based_on` と設計モデル間9種類の計10種類へ写像し、`decomposes` 等の非採用関係は同種間 `contains`、モデル内部情報、または関係探索へ変換する |
| 検証 | 【未適用】JSONパース失敗、スキーマ不一致、候補From/To未解決、型・方向・階層違反、許容外関係、重複・意味重複候補、SRC根拠なし候補を採用前に止める |
...
| `sdd_data_structure.md` の `trace_link` | 【未適用】台帳登録されたリソース間の文書由来、設計意味、実装・検証関係として扱う。relation_type は10種類、方向はrelation_typeごとに固定し、許可ルールと差分属性は `relation_rule_master` を正規メタモデルとして共通利用する |
