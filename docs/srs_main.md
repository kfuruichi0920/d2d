...
| 文書 | 内容 |
| --- | --- |
| [全体詳細設計書](detail_design.md) | 実行時フォルダ構成・成果物配置・Git管理対象・ZIPアーカイブ配置 |
...
| [プラグイン詳細設計書](plugin_detail_design.md) | プラグインソケット種類・データ流れ・呼び出し関係・イベント連携・無効化時の扱い |
...
本ツールは、マイクロアーキテクチャ方式とし、プラグインで実現する機能は個別に有効化・無効化、取替できること。
...
| CORE-003 | プラグインソケット種別ごとの有効化条件、任意連携、設定を定義できること                  |
| CORE-004 | 無効化されたプラグインソケットまたは機能を利用するUI・CLIを非表示または実行不可にできること |
...
| CORE-012 | プロジェクトをまたいでのデータ管理も可能とする（将来機能）                    |
...
| CORE-015 | 実行時フォルダ構成は、[全体詳細設計書](detail_design.md) に従い、ワークスペース単位の設定・一時領域と、プロジェクト単位の原本、成果物フォルダ、設計モデル、アーカイブを分離できること |
...
ZIPアーカイブは、現時点ではGit履歴との連携よりも、過去時点の②抽出データ、③中間データ、④設計モデルを気軽に保存し、現在の成果物との差分を確認するために利用する。将来的にはGit管理の履歴情報と連携できる余地を残す。
...
本ツールは、初期運用で扱う関係性を最小限に絞る。詳細な関係種別は最初から増やさず、レビュー運用で必要性が確認されたものだけを追加する。
...
上記以外の `allocate`、`derive`、`conflict`、`duplicate`、`related`、`phase_trace` 等は初期必須の関係種別としない。必要な場合は、既存5種で表現できない理由、表示・分析での利用目的、レビュー手順を定義してから追加する。
...
プラグインソケット種類、プラグインソケット間のデータ流れ、呼び出し関係、イベント連携、無効化時の扱いは、[プラグイン詳細設計書](plugin_detail_design.md) に定義する。本章では、本ツールが提供するプラグインソケットへの要求と、プラグインプラグに求める共通要求を定義する。

### 19.1 本ツール側のプラグインソケット要求

本ツールは、外部または追加機能として実装されるプラグインプラグを接続するためのプラグインソケットを明示的に提供する。プラグインソケットは、データ階層①〜④、ジョブ管理、成果物管理、レビュー、トレーサビリティ、出力といった本ツールの中核機能を壊さずに拡張するための契約である。

| ID | 要求 |
| --- | --- |
| PLUGIN-HOST-001 | 本ツールは、プラグインを登録、読込、有効化、無効化できるプラグインソケットを提供すること |
| PLUGIN-HOST-002 | 本ツールは、プラグインソケット種別ごとに入力データ、出力データ、イベント、設定、権限、エラー形式を定義できること |
| PLUGIN-HOST-003 | 本ツールは、プラグインが直接ファイルI/OやDB更新を行わず、安全なAPI境界を通じて成果物、設計モデル、設定、ログへアクセスできるプラグインソケットを提供すること |
| PLUGIN-HOST-004 | 本ツールは、プラグインソケット種別ごとの有効化条件、必須機能、任意機能、有効化条件と設定を解決し、無効化された機能を利用するUI・CLIを非表示または実行不可にできること |
| PLUGIN-HOST-005 | 本ツールは、ジョブ管理、イベントバス、成果物管理、レビュー状態管理、LLM送信制御、Git差分確認をプラグインから利用できる共通サービスとして提供すること |
| PLUGIN-HOST-006 | 本ツールは、プラグインのバージョン、対応schema_version、利用するプラグインソケット種別、任意連携するプラグインソケット種別、ライセンス、設定項目をmanifestとして管理できること |
| PLUGIN-HOST-007 | 本ツールは、プラグイン追加時に未解決の有効化条件、schema不整合、権限過多、有効化条件・設定不整合を検出できること |

### 19.2 必須のプラグインソケット種別

以下は単なる実装例ではなく、本ツールとして必要なプラグインソケット種別である。個々のプラグインプラグは、対応するプラグインソケットの要求を満たす機能として設計・実装する。

| プラグインソケット種別 | 本ツールとして必要な理由 | プラグインソケットが必要とするプラグインプラグ機能 | 詳細設計書 |
| --- | --- | --- | --- |
| 文書抽出プラグインソケット | ①原本データから②抽出データを生成するため | Word、Excel、PowerPoint、Visio、PDF、テキスト、Markdown、CSV、TSV、JSON等の形式別抽出 | `document_extractor_plugin_design.md` および形式別詳細設計書 |
| 中間データ処理プラグインソケット | ②抽出データを③成果物単位の中間データへ統合・整理し、必要に応じて設計編集機能を利用するため | チャンク生成、用語抽出、参照リンク、表正規化、図表説明、設計編集機能の呼び出し | `intermediate_processor_plugin_design.md` |
| LLMプロバイダプラグインソケット | 文書抽出、中間データ処理、設計編集、トレーサビリティ、履歴管理から共通的にLLM候補生成を利用できるようにするため | 外部LLM、ローカルLLM、クラウドLLMのプロバイダ接続 | `llm_provider_plugin_design.md` |
| 設計編集プラグインソケット | ③中間データおよび④設計モデルの設計内容を編集・検索・レビュー補助するため | 用語検索、図編集、表編集、モデル編集、シナリオ編集、IF編集、状態遷移編集、PlantUML/Mermaid/SysMLv2等のモデル表現編集 | `design_editor_plugin_design.md` |
| トレーサビリティプラグインソケット | ②抽出データ、③中間データ、④設計モデルのすべての情報単位IDに対する関係表示、検索、分析を共通機能として提供するため | トレースマトリクス、関係クエリ、影響分析、カバレッジ分析 | `trace_plugin_design.md` |
| 履歴管理プラグインソケット | ②抽出データ、③中間データ、④設計モデルの変更前、特定時点、時点間差分を設計編集中などに随時参照できるようにするため | SQLite DB内容比較、SQLite DB履歴差分確認、②/③/④成果物の履歴差分確認 | `history_management_plugin_design.md` |

### 19.3 プラグインプラグの共通要求

プラグインプラグは、本ツールのプラグインソケットに接続される追加機能であり、プラグインソケット側の契約を満たす必要がある。

| ID | 要求 |
| --- | --- |
| PLUGIN-IMPL-001 | プラグインプラグは、対応するプラグインソケット種別、プラグインID、バージョン、対応schema_version、任意連携するプラグインソケット種別、必要な有効化条件をmanifestで宣言すること |
| PLUGIN-IMPL-002 | プラグインプラグは、入力、出力、更新対象、発行イベント、購読イベント、エラー形式を設計書に明記すること |
| PLUGIN-IMPL-003 | プラグインプラグは、①原本、②抽出データ、③中間データ、④設計モデルのどの階層を読み書きするかを明示すること |
| PLUGIN-IMPL-004 | プラグインプラグは、正本、派生成果物、索引、キャッシュ、一時ファイルの区別を壊さないこと |
| PLUGIN-IMPL-005 | プラグインプラグは、LLM出力を確定情報として直接反映せず、候補、レビュー状態、根拠リンク、ログを分離して扱うこと |
| PLUGIN-IMPL-006 | プラグインプラグは、処理失敗時に可能な範囲の部分成果物、警告、未対応理由、再実行条件を記録できること |
| PLUGIN-IMPL-007 | プラグインプラグは、UIスレッドをブロックせず、長時間処理をジョブとして実行できること |
| PLUGIN-IMPL-008 | プラグインプラグは、商用利用上のライセンス、外部送信有無、機密情報の扱いを明示すること |

### 19.4 プラグイン設計書要求

個々のプラグインソケットに対応するプラグインプラグの詳細は、`XXXX_plugin_design.md` 形式の設計書として定義する。設計書は、単なる名称一覧ではなく、そのプラグインがプラグインソケット側要求にどう応えるかを機能として明記する。

| ID | 要求 |
| --- | --- |
| PLUGIN-DESIGN-001 | プラグイン設計書は、対象プラグインソケット、目的、責務範囲、非責務範囲を明記すること |
| PLUGIN-DESIGN-002 | プラグイン設計書は、入力データ、出力データ、成果物フォルダ構成、manifest項目、schema_versionを明記すること |
| PLUGIN-DESIGN-003 | プラグイン設計書は、利用するプラグインソケット種別、任意連携するプラグインソケット種別、発行イベント、購読イベントを明記すること |
| PLUGIN-DESIGN-004 | プラグイン設計書は、UI、CLI、ジョブ、ログ、レビュー状態、エラー処理、再実行条件を明記すること |
| PLUGIN-DESIGN-005 | プラグイン設計書は、LLM利用有無、外部送信有無、候補情報の扱い、人間レビューとの関係を明記すること |
| PLUGIN-DESIGN-006 | プラグイン設計書は、Git管理対象、Git対象外、ZIPアーカイブ対象、差分比較対象を明記すること |
