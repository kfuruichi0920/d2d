...
| FORM-021 | SysMLv2モデル編集を設計編集プラグインソケットの対象として扱えること      |
...
以下は本ツールとして必要なプラグインソケット種別である。個々のプラグインプラグは、対応するプラグインソケットの要求を満たす機能として設計・実装する。

| プラグインソケット種別 | 本ツールとして必要な理由 | プラグインソケットが必要とするプラグインプラグ機能 | 詳細設計書 |
| --- | --- | --- | --- |
| 文書抽出プラグインソケット | ①原本データから②抽出データを生成するため | Word、Excel、PowerPoint、Visio、PDF、テキスト、Markdown、CSV、TSV、JSON等の形式別抽出 | [文書抽出プラグインソケット設計書](document_extractor_plugin_design.md) および形式別詳細設計書 |
| 中間データ処理プラグインソケット | ②抽出データを③成果物単位の中間データへ統合・整理し、必要に応じて設計編集機能を利用するため | チャンク生成、用語抽出、参照リンク、表正規化、図表説明、設計編集機能の呼び出し | [中間データ処理プラグインソケット設計書](intermediate_processor_plugin_design.md) |
| LLMプロバイダプラグインソケット | 文書抽出、中間データ処理、設計編集、トレーサビリティ、履歴管理から共通的にLLM候補生成を利用できるようにするため | 外部LLM、ローカルLLM、クラウドLLMのプロバイダ接続 | [LLMプロバイダプラグインソケット設計書](llm_provider_plugin_design.md) |
| 設計編集プラグインソケット | ③中間データおよび④設計モデルの設計内容を編集・検索・レビュー補助するため | 用語検索、図編集、表編集、モデル編集、シナリオ編集、IF編集、状態遷移編集、PlantUML/Mermaid/SysMLv2等のモデル表現編集 | [設計編集プラグインソケット設計書](design_editor_plugin_design.md) |
| トレーサビリティプラグインソケット | ②抽出データ、③中間データ、④設計モデルのすべての情報単位IDに対する関係表示、検索、分析を共通機能として提供するため | トレースマトリクス、関係クエリ、影響分析、カバレッジ分析 | [トレーサビリティプラグインソケット設計書](trace_plugin_design.md) |
| 履歴管理プラグインソケット | ②抽出データ、③中間データ、④設計モデルの変更前、特定時点、時点間差分を設計編集中などに随時参照できるようにするため | SQLite DB内容比較、SQLite DB履歴差分確認、②/③/④成果物の履歴差分確認 | [履歴管理プラグインソケット設計書](history_management_plugin_design.md) |
