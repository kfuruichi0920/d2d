...
| EXT-009 | 【未適用】Excelセル、結合セル、行列構造を抽出できること           |
...
④設計モデルは、文書および文書内位置を扱う文書世界と、文書から抽出・構造化した設計モデル世界を分離し、設計要素と設計要素間の関係も分離して管理する。設計要素の分類は以下の13分類とし、各設計要素は自らの分類を属性として保持できること。設計要素は `entity_registry` に共通登録し、種別固有の情報は `resource_*` 詳細テーブルで管理する。
...
| 構造情報 | STRUCT | システム、装置、ソフトウェア、サービス、プロセス、モジュール、コンポーネント、タスク | `resource_model`、`resource_code`、`resource_metadata` |
| 振舞情報 | BEH | シナリオ、処理手順、イベント、アクション | `resource_scenario`、`resource_model` |
| 状態情報 | STATE | 状態モデル、状態、状態遷移、遷移条件 | `resource_state_transition` |
...
| MODEL-003 | 【一部適用】 `SRC` は設計意味関係の主対象ではなく、文書トレーサビリティ関係 `based_on` の対象として扱うこと |
| MODEL-004 | `STATE`、`IF`、`DATA`、`MGMT`、`VERIF` 等の所有者・管理主体・ライフサイクル責任は `owner_uid` 属性で表現できること |
| MODEL-005 | 【一部適用】 `owner_uid` と `allocated_to` を区別し、前者は所有者・管理主体、後者は機能・構造・振舞・状態間の責務割当を表すこと |
...
| MODEL-009 | 候補セットを採用する際は、要素候補と関係候補を同一トランザクションで検査し、許容関係、重複、未解決参照、生成元チャンクへの根拠リンク不足を検出できること |
| MODEL-010 | 【一部適用】 関係候補では起点・終点分類に対して `relation_rule_master` が許容する関係種別だけを選択肢とすること。LLMが許容外値を返した場合は初期値を保持して行単位で警告し、採用を禁止すること |
| MODEL-011 | 【未適用】 設計要素を、文書・入力層（SRC/STD）、要求層（REQ/CST）、論理設計層（FUNC/STRUCT/BEH/STATE）、情報・契約層（IF/DATA）、実装・検証層（IMPL/VERIF）、管理層（MGMT）として扱えること。STRUCTは物理構造に限定せず、論理構造、ソフトウェア構造、実行構造、物理構造を下位種別で識別できること |
| MODEL-012 | 【未適用】 BEHの処理順序・条件分岐等、STATEの遷移、IFの入出力・パラメータ、DATAの項目・型・構造、VERIFの試験手順・期待結果等は各モデル内部で管理し、独立した関係種別へ増殖させないこと |

### 9.2 関係性管理

【一部適用】`trace_link.relation_type` は、関係種別の増加による意味重複、入力判断の揺れ、検索・分析時の複雑化を避けるため、文書由来専用の `based_on` と設計モデル間の9種類を合わせた以下の10種類に限定する（詳細DDLは `sdd_data_structure.md` 参照）。関係方向はrelation_typeごとに固定し、逆方向参照は逆関係を重複登録せず検索時に解決する。

| 関係種別 | 分類 | 用途 | 正規方向・特性 |
| --- | --- | --- | --- |
| `based_on` | 文書トレーサビリティ関係 | 原本、抽出データ、中間データ等の文書世界と、そこから作成した情報との由来を表す。設計モデル間の意味関係には使用しない | SRC→SRC、設計要素→SRC |
| `satisfies` | 設計意味関係 | 機能・構造・振舞・状態・IF・データ・実装が、規範・要求・制約を満たすことを表す | FUNC/STRUCT/BEH/STATE/IF/DATA/IMPL → STD/REQ/CST |
| `allocated_to` | 設計意味関係 | 機能・構造・振舞を、それに対応する構造・振舞・状態へ割り当てることを表す | FUNC→STRUCT/BEH/STATE、STRUCT→BEH、BEH→STATE |
| `verifies` | 実装・検証関係 | 試験、レビュー、解析、検証情報が、規範・要求・制約・設計要素・管理情報・実装を検証することを表す | VERIF → 検証対象 |
| `contains` | 設計意味関係 | 同じ分類の上位要素が下位要素を、内部要素、構成要素、部分要素または詳細要素として含むことを表す | 上位要素 → 同分類の下位要素 |
| `implements` | 実装・検証関係 | ソースコード、設定、DDL、モデル等の実装要素が設計要素を実装することを表す | IMPL → FUNC/STRUCT/BEH/STATE/IF/DATA |
| `uses` | 設計意味関係 | 許可された設計要素間で、処理、参照、通信、状態判定、データ利用または再利用の対象が明確なことを表す | 利用元 → 利用対象 |
| `calls` | 実装・検証関係 | 実装要素が別の実装要素を実行時に呼び出すことを表す | IMPL → IMPL |
| `conflicts_with` | 設計意味関係 | SRCを除く設計要素間の競合または矛盾可能性を表す | 対称関係。DBには一方向だけ保存し検索時に双方向化 |
| `relates_to` | 暫定・補助 | 正式な関係種別を決定できない場合の一時的・補助的な関連を表す | 設計要素 → 設計要素 |

`decomposes`、`connects`、`input_of`、`output_of`、`transitions_to`、`precedes`、`depends_on`、`supersedes`、`affects`、`derived_from`、`normalized_from`、`owns`、`realizes`、`inputs`、`outputs`、`constrains`、`impacts`、`refines` は relation_type として採用しない。代替表現は以下とする。

| 非採用 relation_type | 代替表現 |
| --- | --- |
| `derived_from` / `normalized_from` | 文書世界の由来に限り `based_on + basis_kind` |
| `owns` | `owner_uid` |
| `realizes` | `satisfies` または `implements` |
| `depends_on` | 許可された `uses`、IMPL間の `calls`、または関係探索により導出 |
| `inputs` / `outputs` / `input_of` / `output_of` | IF、BEH等のモデル内部情報として保持し、モデル間利用が必要な場合だけ許可された `uses` を併用 |
| `constrains` | 設計要素が制約を `satisfies` する関係で表現 |
| `impacts` / `affects` | 関係探索により導出 |
| `decomposes` / `refines` | 同じ分類の上位要素から下位・詳細要素への `contains` |
| `connects` / `transitions_to` / `precedes` | IF、STATE、BEH等のモデル内部情報として保持 |
| `supersedes` | 関係ではなく版・有効期間・Git履歴で管理 |

**注意**: 文書構成上の親子関係（章節配下等）は通常 `extracted_document.structure_json` および `intermediate_document.structure_json` 内のJSON構造として管理し、明示的なSRC要素としてモデル化する場合だけSRC間の `contains` を使用する。設計意味上の階層・包含・部分化・詳細化は、同じ分類間の `contains` に統一する。状態遷移、処理順序、IF入出力等は各モデル内部で管理する。

### 9.3 関係属性

【一部適用】`trace_link` は、関係種別だけで意味を増やさず、関係自体を識別可能なエンティティとして共通属性とrelation_type固有属性を保持する。

| 対象 | 属性 | 値・用途 |
| --- | --- | --- |
| 共通 | `uid`、`from_uid`、`to_uid`、`relation_type` | 関係ID、正規方向の起点・終点、関係種別 |
| 共通 | `status` | `active / provisional / deprecated / rejected` |
| 共通 | `confidence` | 自動抽出・推論結果の信頼度 |
| 共通 | `created_by` | `human / llm / rule / import / static_analysis` |
| 共通 | `review_status` | `unreviewed / llm_proposed / human_reviewed / confirmed / rejected` |
| 共通 | `rationale` | 関係設定の理由・根拠説明 |
| 共通 | `created_at`、`updated_at` | 作成・更新日時 |
| 共通 | `valid_from`、`valid_to`、`version` | 関係の有効期間と版 |
...
| `allocated_to` | `allocation_kind` | `structure / behavior / state` |
| `allocated_to` | `allocation_role` | `primary / supporting` |
| `uses` | `usage_kind` | `input / output / read / write / update / publish / subscribe` |
| `conflicts_with` | `conflict_kind` | `logical / requirement / constraint / resource / timing / safety / security / interface / data / implementation / priority` |
| `conflicts_with` | `context_uid`、`condition`、`severity`、`conflict_status`、`resolution_note` | 競合の文脈、条件、重大度、状態（`suspected / confirmed / resolved / accepted / rejected`）、解消方針 |
| `relates_to` | `status`、`reason` | `status=provisional` とし、理由は `relation_type_unknown / insufficient_information / awaiting_review / imported_legacy_relation` のいずれか |

### 9.4 設計要素間の許容関係

【未適用】以下を正規の許可関係表とし、`relation_rule_master` の初期データおよび実行時検査はこの表と一致させる。

| relation_type | 許容 Source | 許容 Target | ルール |
| --- | --- | --- | --- |
| `based_on` | SRC/STD/REQ/CST/FUNC/STRUCT/BEH/STATE/IF/DATA/VERIF/MGMT/IMPL | SRC | 文書由来専用。①〜③の文書・項目はSRCとして扱い、④設計モデル間には使用しない |
| `satisfies` | FUNC/STRUCT/BEH/STATE/IF/DATA/IMPL | STD/REQ/CST | 要求・制約・規範への充足関係。要求側からは逆引き表示する |
| `allocated_to` | FUNC/STRUCT/BEH | FUNC→STRUCT/BEH/STATE、STRUCT→BEH、BEH→STATE | 表記した組合せ以外は禁止する |
| `verifies` | VERIF | STD/REQ/CST/FUNC/STRUCT/BEH/STATE/IF/DATA/MGMT/IMPL | 検証情報から検証対象へ向ける。VERIF間の階層は `contains` を使う |
| `contains` | SRC/STD/REQ/CST/FUNC/STRUCT/BEH/STATE/IF/DATA/VERIF/MGMT/IMPL | Sourceと同じ分類 | 階層、包含、部分化、詳細化。自己包含・循環を禁止し、原則として正規の直接親は一つとする |
| `implements` | IMPL | FUNC/STRUCT/BEH/STATE/IF/DATA | IMPL→REQ/CSTは `implements` では表さず、直接充足は `satisfies` を使う |
| `uses` | FUNC/STRUCT/BEH/STATE/IF | FUNC→FUNC/IF/DATA、STRUCT→BEH/DATA、BEH→BEH/STATE/IF/DATA、STATE→BEH/STATE/IF/DATA、IF→DATA | 設計上明確な利用対象だけに使用し、割当・包含・検証・実装・呼出しの代用にしない |
| `calls` | IMPL | IMPL | 実装要素間の実行時呼出しに限定する |
| `conflicts_with` | STD/REQ/CST/FUNC/STRUCT/BEH/STATE/IF/DATA/VERIF/MGMT/IMPL | 同じ12分類の任意要素 | 対称関係として1件だけ保存し、検索時に双方向で扱う。SRCは対象外 |
| `relates_to` | STD/REQ/CST/FUNC/STRUCT/BEH/STATE/IF/DATA/VERIF/MGMT/IMPL | 同じ12分類の任意要素 | 暫定リンク専用。`status=provisional` と理由を必須とし、レビュー後は正式なrelation_typeへ置換する |

### 9.5 関係登録・品質バリデーション

| ID | 要求 |
| --- | --- |
| MODEL-013 | 【未適用】 関係登録時および候補採用時に、relation_typeごとのSource分類、Target分類、方向、必須属性を `relation_rule_master` で検証し、許可表にない組合せを保存しないこと |
| MODEL-014 | 【未適用】 `contains` は自己包含、循環包含、異種分類間包含を禁止し、必要に応じて正規の直接親を一つに制約できること |
| MODEL-015 | 【未適用】 同じ `source_uid + relation_type + target_uid` の重複関係を禁止すること。`conflicts_with` は一つの関係を双方向表示し、逆向きの重複を作成しないこと |
| MODEL-016 | 【未適用】 `FUNC uses STRUCT/BEH/STATE`、`VERIF uses X`、`IMPL uses IMPL`、異種分類間 `contains` 等の意味重複・方向逆転を検出し、正しいrelation_typeへの変更候補を提示すること |
| MODEL-017 | 【未適用】 `relates_to` の件数を設計モデル間関係総数で除した暫定関係率を算出でき、暫定関係を定期的なレビュー対象として抽出できること |
| MODEL-018 | 【一部適用】 relation_typeごとの許可ルールはアプリコードへ分散してハードコードせず、`relation_rule_master` を正規メタモデルとして保存前検査、UI候補制御、LLM出力検証、品質分析から共通利用すること |
