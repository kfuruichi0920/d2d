...
| EXT-027 | 【一部適用】PDF文書は、ページ単位の画像、ページ寸法、テキスト、表、図・画像、数式、タイトル・キャプション、各要素の座標範囲を抽出できること（数式領域の自動検出のみ未対応。手動種別変更とVision OCRで代替） |
...
| EXT-063 | Excel原本の「抽出グループ候補を生成」と「抽出グループ候補を確認」は並べて表示し、候補未生成時は確認を、生成済み時は生成を無効化すること。候補確認は②抽出データ生成後も `stage://source` の原本詳細から何度でも開けること |
| EXT-064 | Excel候補の採否は「採用」「不採用」の2値とし、自動生成、手動追加、範囲LLM追加の初期値をすべて「採用」とすること。候補種別は既定を `text` とし、原本構造から明確に判定できる表、リスト、図等だけを対応種別とすること |
| EXT-065 | Excel候補の次回再取込差分判定用情報は独立した保存操作を設けず、「抽出を実行して②を生成」のトランザクションで採用・不採用を含む全候補へ毎回更新すること。次回差分は②生成まで完了した同名原本の候補だけを基準とすること |
| EXT-066 | Excelシートプレビューのキーボード選択はカーソルが見える最小範囲へスクロール追従し、Ctrl+矢印はExcel同様に現在領域端または次の非空セルへ移動すること。マウスドラッグによる複数セル選択はシート保持範囲へ制限し、領域外移動やダブルクリックを含む連続操作で画面を破損しないこと |
| EXT-067 | Excel候補詳細の範囲内文字列は右ペイン幅内で折り返し、利用者が高さを変更できること。開始セル、終了セル、種別、採否の入力高さはペイン高に引き伸ばさず、余剰高さは範囲内文字列の表示へ割り当てること |
