...
| UI-015 | 汎用インパクト分析ビューを提供すること。各列へ任意の複数Resource集合を配置し、左右への列追加／削除とドラッグ並替え、独立縦スクロール、複数関係種別、離れた列を含む方向付きリンク、リンク表示ON/OFF、クリック／上下キー／Shiftによる項目複数選択、Secondary連携、全列への連鎖強調、関連項目限定表示、階層の折畳／展開と親へのリンク集約、項目／リンクTooltip、表示領域限定リンク描画、名前付き構成の複数保存・復元、固有URIによる複数タブ表示を提供すること。トレースマトリクスも固有URIで複数タブ表示できること |
