...
| EXT-030 | PDF文書の図・表・数式・不鮮明テキストは、選択領域単位でOCRまたはLLM補正の候補を生成し、人間レビュー後に抽出結果へ反映できること |
...
④設計モデルは、設計要素と設計要素間の関係を分離して管理する。設計要素は `entity_registry` に共通登録し、種別固有の情報は `resource_*` 詳細テーブルで管理する。
...
| UI-032 | PDF抽出レビュービューは、選択領域のプロパティ、表データ編集、OCR/LLM補正候補、Markdown/JSON/表プレビュー、ジョブ・LLMログを同一Workbench内で確認できること |
