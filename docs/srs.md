...
| UI-046 | 上部Pipeline Navigatorのステージを選択すると、①原本は取込済み原本一覧と読取専用詳細、②抽出は抽出文書一覧と独自プレビュー、③中間は開発フェーズ－成果物階層一覧と独自プレビュー、④モデルは設計モデル一覧をEditor Areaへ開くこと。①原本のファイル表示はWindowsに関連付けられた外部アプリへ委譲すること |
