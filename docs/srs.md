...
| EDIT-072 | 各種テキスト入力欄は通常時に表示文章のプレビューだけを背景色付きで表示し、編集ボタンまたはプレビュー選択中のショートカットから、Markdown編集、プレビュー、構造化データ、用語候補、用語候補(LLM)、校正・正規化(LLM)、入力欄設定を集約した編集画面を開くこと |
| EDIT-073 | Secondary Side BarのProperties、Relations、Review、Dictionaryは、開閉状態を変更しても表示順を変更しないこと |
| EDIT-074 | Resource編集および各種テキスト編集はモーダル表示から同一ResourceのEditor Areaタブへ統合でき、ダイアログに統合操作を表示すること |
