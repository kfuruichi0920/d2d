...
| 論理設計 | `model_action` | ACTION | 振舞 | シナリオ、処理手順、イベント、アクション等、時間順序を持つ振舞 |
...
| MODEL-010 | LLM候補はレビューで採用されるまで④の確定情報にせず、採用時に候補要素、候補関係、根拠リンクを一括検査・保存すること |
...
| 関係種別 | 用途・正規方向 |
| --- | --- |
| `based_on` | `model_*` → ①原本、②抽出データ、③中間データまたはチャンク。設計モデル間の意味関係には使用しない |
...
| MODEL-023 | 設計モデル関係ごとに日本語の名称、定義、必須属性、有効・無効を管理画面で表示・編集できること |
...
| UI-060 | 既存のSettings画面にオントロジー設定管理を設け、モデル定義、関係定義、有効状態、追加、許容マトリクス、オントロジーバージョン確定を操作できること |
