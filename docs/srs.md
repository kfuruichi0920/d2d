...
| UI-049 | Workbench各画面はCtrl+Fおよび常時視認可能な検索ボタンから表示中文字列を検索し前後一致へ移動できること。検索ボタンはTitle Bar右側にデスクトップIDE風のアイコンボタンとして表示すること。すべての操作ボタンはhoverまたはキーボードfocus時に操作説明をTooltip表示すること。文書プレビューは形式非依存の共通設定としてパーツ種別、セクション、要素IDを個別に表示切替できること |
