...
| 24 | `table_resource.source_hash` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | メタ情報 | 表差分検出 |
| 25 | `requirement.requirement_type` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 |  |
| 26 | `requirement.body_text` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 表示 | Yes | No | No | 将来候補 | RAGは初期導入しない |
| 27 | `requirement.priority` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 |  |
| 28 | `requirement.acceptance_criteria` | 対象 | 部分一致 | FTS(MeCab) | 詳細のみ表示 | Yes | No | No | 将来候補 | RAGは初期導入しない |
| 29 | `use_case.primary_actor` | 対象 | 完全一致/部分一致 | 通常 | 表示 | Yes | Yes | No | 将来候補 | RAGは初期導入しない |
| 30 | `use_case.main_flow_text` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 表示 | Yes | No | No | 将来候補 | RAGは初期導入しない |
| 31 | `state_definition.state_name` | 対象 | 完全一致/部分一致 | 通常 | 表示 | Yes | Yes | No | 将来候補 | RAGは初期導入しない |
| 32 | `state_transition.from_state_uid` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | フィルタ条件 | 遷移探索 |
| 33 | `state_transition.to_state_uid` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | フィルタ条件 | 遷移探索 |
| 34 | `state_transition.event_text` | 対象 | 部分一致 | 通常 | 表示 | Yes | Yes | No | 将来候補 | RAGは初期導入しない |
| 35 | `trace_link.from_uid` | 対象 | ID検索 | 通常/複合 | 詳細のみ表示 | Yes | No | No | フィルタ条件 | 前方探索 |
| 36 | `trace_link.to_uid` | 対象 | ID検索 | 通常/複合 | 詳細のみ表示 | Yes | No | No | フィルタ条件 | 逆引き |
| 37 | `trace_link.relation_type` | 対象 | 完全一致 | 通常/複合 | 表示 | Yes | Yes | No | フィルタ条件 | 関係種別 |
| 38 | `trace_link.rationale` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 詳細のみ表示 | Yes | No | No | 将来候補 | RAGは初期導入しない |
| 39 | `llm_run_ref.input_ref_uid` | 対象 | ID検索 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 入力追跡 |
| 40 | `llm_run_ref.status` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 |  |
...
`entity_type` は `entity_registry` 上の種別識別子であり、CHECKの許容値は `entity_registry` 以外の管理対象物理テーブル名と一致させる。詳細テーブルを持つ主要エンティティは、原則として `entity_type` と詳細テーブルを1対1で対応させる。`project` は最上位管理単位であり、共通台帳登録は可能だが詳細テーブル側から `entity_registry` へFK接続する通常詳細テーブルとは扱いを分ける。整合性はDBトリガーではなくアプリで保証する。
