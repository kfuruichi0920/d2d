...
| 関係 | `trace_link` |
| LLM | `llm_run_ref`、`prompt_template`、`llm_provider_config`、`llm_request_log` |
...
| `uid` | TEXT | PK |
| `name` | TEXT | NOT NULL |
| `schema_version` | TEXT | NOT NULL。新規プロジェクトは `2.0.0` |
...
| `model_action` | `trigger`、`preconditions`、`steps`、`postconditions` |
...
### 4.10 ontology_relation_definition
...
### 4.11 ontology_relation_allowance
...
新規プロジェクトでは、schema 2.0.0のDDL実行後に次を行う。
