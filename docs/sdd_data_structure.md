...
erDiagram
    ...
    DESIGN_ELEMENT ||--o{ DESIGN_RELATION : from
    DESIGN_ELEMENT ||--o{ DESIGN_RELATION : to

    PROJECT ||--o{ TRACE_SUBJECT : has
    TRACE_SUBJECT ||--o{ TRACE_LINK : from_subject
    TRACE_SUBJECT ||--o{ TRACE_LINK : to_subject
...
| project_id | プロジェクトID |
| artifact_id | 成果物ID |
| item_type | title / text / table / figure / model / state_transition / interface / verification |
| state | 作成中 / レビュー中 / 確定 / 棄却 |
...
| source_doc_node_id | 根拠となる文書ノードID |
| state | 作成中 / レビュー中 / 確定 / 棄却 |
| created_by | human / rule / llm |
| llm_run_ref_id | LLM利用時の実行ログ参照 |
| created_at | 作成日時 |
...
| relation_type | parent_child / based_on / satisfy / verify / allocate / realize / depend / derive / conflict / duplicate / related |
| description | 関係説明 |
| state | 作成中 / レビュー中 / 確定 / 棄却 |
| created_by | human / rule / llm |
| llm_run_ref_id | LLM利用時の実行ログ参照 |
...
| project_id | プロジェクトID |
| entity_type | original_file / original_location / extracted_item / intermediate_item / design_element / design_relation / artifact |
| entity_id | 実体ID |
...
### 18.5 design_relation と trace_link を分離する理由
...
| SRS要求 | 本設計での対応 |
| --- | --- |
| CORE-010〜015 | `project` を最上位単位とし、プロジェクト単位で設定、DB、成果物フォルダ、アーカイブ、ログを関連付ける |
