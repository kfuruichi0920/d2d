...
| `2.3.0` | `excel_extraction_draft`を追加 | Excel物理抽出・候補編集を②正本と分離して保持 |
| `2.4.0` | Excelドラフトに再取込差分列を追加 | 直前原本と候補UID継承の判定根拠を保持 |
...
| 抽出 | `excel_extraction_draft`、`extracted_document`、`extracted_item` |
...
| `uid` | TEXT | PK |
| `name` | TEXT | NOT NULL |
| `schema_version` | TEXT | NOT NULL。新規プロジェクトは `2.4.0` |
...
- `based_on`: ①〜③、チャンク、`resource_*`、`model_*` の由来・根拠に使用する。根拠Resource同士を許可し、両端が `model_*` の場合だけ禁止する。
...
### 4.13 excel_extraction_draft

Excel候補はユーザー確定まで②正本から分離する。physical_json はセル・直接装飾・DrawingML等の原本保存情報、candidates_json は編集可能候補を保持する。predecessor_source_document_uid は同名Excelの直前ドラフト、diff_json はシートと候補の差分・候補UID継承判定を保持する。曖昧な分割・統合を自動継承しない。

| カラム | 型 | 制約・用途 |
| --- | --- | --- |
| source_document_uid | TEXT | PK/FK。①Excel原本 |
| status | TEXT | generated / editing / confirmed / failed |
| physical_json / candidates_json | TEXT | JSON妥当性をCHECK |
| predecessor_source_document_uid | TEXT | 再取込の比較元 |
| diff_json | TEXT | NULLまたは妥当なJSON |
| last_llm_run_uid | TEXT | 最後のLLM実行証跡 |
| confirmed_extracted_document_uid | TEXT | 確定後の②抽出文書 |
| created_at / updated_at / confirmed_at | TEXT | 監査時刻 |
## 5. 検索・トレース・出力
...
新規プロジェクトでは、schema 2.0.0の基底DDLを実行し、2.4.0までのmigrationを適用したうえで次を行う。
