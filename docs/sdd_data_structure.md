...
| 5 | スキーマバージョン | `schema_version` | DBスキーマバージョン | SQLite: TEXT / App: semver string | No | Yes | No | なし | `'1.0.0'` | なし | 原則不可 | アプリ | マイグレーション判定に使う |
| 6 | 作成日時 | `created_at` | 作成日時 | SQLite: TEXT / App: ISO 8601 string | No | Yes | No | なし | `CURRENT_TIMESTAMP` | ISO 8601 | 不可 | 現在時刻 | UTC推奨 |
| 7 | 更新日時 | `updated_at` | 最終更新日時 | SQLite: TEXT / App: ISO 8601 string | No | Yes | No | なし | `CURRENT_TIMESTAMP` | ISO 8601 | 可 | アプリ | SQLiteトリガーまたはアプリで更新 |
...
| 6 | 状態 | `status` | 現在状態 | SQLite: TEXT / App: enum string | No | Yes | No | なし | `'draft'` | `draft/review/approved/rejected/archived` | 可 | アプリ | 履歴ではなく現在状態 |
...
| `fts_entity_text` | `entity_registry`, detail text columns | MeCabで形態素解析した検索用本文 | FTS | 日本語全文検索 | 初期推奨 |
...
`ON DELETE CASCADE` は、共通台帳の削除に連動して詳細行を消すために採用する。ただし、UIでは誤削除を避けるため、通常操作は `status='archived'` による論理的な非表示を優先する。
...
| 論点 | 推奨 | 理由 | 後で見直す条件 |
| -- | -- | -- | ------- |
| UUIDv7をTEXTで持つかBLOBで持つか | 初期は `TEXT` | SQLite CLI、JSON、Markdown出力、デバッグで扱いやすい | データ件数が大きくなり、DBサイズや索引性能が問題になった場合 |
| `row_id INTEGER PRIMARY KEY` を併用するか | 初期は併用しない | `uid` で外部参照とGit差分の安定性を確保できる | 大量データでJOIN性能が問題になり、実測で効果が確認できた場合 |
| FTS5を初期導入するか | 初期検索機能として導入候補にする | MeCabで形態素解析した検索用テキストをFTS5に登録すれば、日本語本文検索の性能と精度を上げやすい | MeCab辞書、更新タイミング、検索対象列の実装コストが初期スコープに収まらない場合 |
| 日本語全文検索をどう扱うか | MeCabで形態素解析した結果をFTS5登録用テキストとして保持する | SQLite標準FTSだけに任せるより、日本語の語境界を扱いやすい | 辞書管理、表記揺れ、専門用語追加の運用が固まった場合 |
| blobをDB内に持つか外部ファイルにするか | 外部ファイルにする | DB肥大化、Git差分困難、バックアップ負荷を避ける | 小さなサムネイル等をDB内キャッシュしたい場合 |
| Git差分確認用エクスポート形式 | DB to Text は JSONL、テーブル毎、`uid` 昇順 | 機械処理しやすく、行単位差分も安定しやすい | 人間向けレビュー用にMarkdownビューやHTMLレポートが必要になった場合 |
| `entity_type` と詳細テーブルの整合性 | 初期はアプリで保証し、必要ならトリガー追加 | SQLiteのCHECKだけでは詳細テーブル存在までは保証しにくい | 複数ツールが直接DBを書き込むようになった場合 |
| 論理削除を採用するか物理削除を採用するか | 通常操作は `status='archived'`、保守操作は物理削除 | 誤削除防止とGit履歴確認を両立する | DBサイズ削減、個人情報削除、原本再取込が必要な場合 |
| 詳細テーブルに `project_id` を持つか | 持たせない | `entity_registry.project_uid` と重複し不整合源になる | 詳細テーブル単独の大量検索で実測上の性能問題が出た場合 |
| `ON DELETE CASCADE` をどこまで使うか | 台帳から詳細への削除はCASCADE、意味破壊する参照はRESTRICT | 共通台帳と詳細の孤児行を防ぎつつ、状態遷移等の意味破壊を防ぐ | UIで削除操作の粒度を細かく設計した後 |
| `code` 採番方法 | prefix + 6桁ゼロ埋め。欠番許容、再採番禁止。同時編集は考慮しない | 人間が読みやすく、Git差分や外部参照で安定する。競合は人と運用でカバーする前提にする | 複数プロセス同時編集を正式にサポートする場合 |
| PostgreSQL移行余地 | SQLite標準型と単純なFK/INDEX中心にする | PostgreSQL移行時にDDL変換しやすい | JSON型、全文検索、ベクトル検索をDBネイティブに寄せる場合 |

## 10. 次に設計すべき項目

| 項目 | 内容 | 優先度 |
| --- | --- | --- |
| 表示コード採番仕様 | `REQ-000001` 等のprefix、6桁ゼロ埋め、欠番許容、再採番禁止を仕様化する。同時編集競合は運用でカバーする | 高 |
| `entity_type` 一覧 | 要求、UC、状態、状態遷移以外の設計要素種別を確定する | 高 |
| DB to Text 形式 | JSONL、テーブル毎、`uid` 昇順、1行1レコードの出力仕様を定義する | 高 |
| blob manifest 仕様 | blobの相対パス、ハッシュ、生成元、削除検出方法 | 中 |
| RAG導入判断 | 初期導入しない。将来導入時に本文、根拠、条件、メタ情報、フィルタ条件の列マッピングを再設計する | 低 |
| FTS5検証 | MeCab形態素解析結果をFTS5に登録し、LIKE検索との差を検証する | 高 |
| トリガー採否 | `updated_at` 自動更新、`entity_type` と詳細テーブル整合性チェック | 中 |
| マイグレーション方針 | `schema_version` の上げ方、DDL適用順、バックアップ方法 | 中 |
| 削除操作設計 | archive、物理削除、blob削除、参照中データ削除のUI/DBルール | 中 |
| PostgreSQL移行メモ | 型変換、UUID型、全文検索、JSONB、ベクトル検索への移行候補 | 低 |
