...
| `resource_figure` | 図リソース | 画像URI、画像ハッシュ、図種別、幅、高さ、バイトサイズ、画像形式、説明 | `uid` | `entity_registry(uid)` | 画像実体はDB外blob領域 |
...
| `resource_figure` | `uid`, `image_uri`, `image_hash`, `figure_kind`, `width`, `height`, `byte_size`, `image_format`, `description`, `caption_uid` | 図リソース | 画像実体はDB外。旧OCR・オブジェクト列はUI・新規保存で使用しない |
...
|  7 | サイズ | `byte_size` | 画像ファイルのバイトサイズ | INTEGER / int | No | No | No | なし | NULL | `byte_size >= 0` | 原則不可 | 抽出 | |
|  8 | 画像フォーマット | `image_format` | PNG、JPEG等 | TEXT / string | No | No | No | なし | NULL | なし | 原則不可 | 抽出 | |
|  9 | 説明 | `description` | 文書中の図説明 | TEXT / Markdown | No | No | No | なし | NULL | なし | 可 | 人/LLM支援 | 図をLLM入力可能 |
| 10 | キャプションUID | `caption_uid`    | 対応するlabel/text    | TEXT / UUIDv7 string  | No  | No  | No  | `entity_registry(uid)` | NULL    | UUIDv7形式                                      | 可    | アプリ             | 図タイトル                           |
