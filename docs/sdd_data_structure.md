...
| 版数 | 変更内容（migrations.ts の description） |
| --- | --- |
| 1.0.0 | 初期スキーマ（本書 §7 DDL案 相当） |
| 1.1.0 | resource_table_cell 別テーブルの追加（TBD-04） |
| 1.2.0 | 成果物設定への開発フェーズ紐付け追加（P7-1） |
| 1.3.0 | チャンク追加プロンプトの追加（MID-032） |
| 1.4.0 | チャンク表示順の追加（MID-031） |
| 1.5.0 | Entityアーカイブ状態の追加（UI-047） |
| 1.6.0 | セマンティック入力支援データの追加（EDIT-057〜071） |
| 1.7.0 | 成果物名のフェーズ単位一意化（CORE-013） |
| 1.8.0 | LLM 実行への生送受信ログ参照の追加（W12） |
...
| セマンティック入力 | `semantic_text` | 入力欄単位で原文、現在の表示文章、入力ポリシーを分離保持する |
| セマンティック入力 | `semantic_reference` | 表示文章内の文字範囲と辞書用語・設計モデルUID、表示方法、関係候補、承認状態を保持する |
| セマンティック入力 | `semantic_normalization_history` | 正規化・置換の変更前後、方法、承認・棄却・取消し状態を保持する |
...
| `resource_text` | テキストリソース | 本文、テキスト役割、言語、補足対象Resource | `uid` | `entity_registry(uid)` | 本文・注記・備考等 |
| `resource_list` | リストリソース | リスト種別、項目数、項目Markdown、最大階層 | `uid` | `entity_registry(uid)` | Markdownリストを保持しResource Editorでプレビュー |
| `resource_figure` | 図リソース | 画像URI、画像ハッシュ、図番号、キャプション、図種別、幅、高さ、バイトサイズ、画像形式、説明 | `uid` | `entity_registry(uid)` | 画像実体はDB外blob領域 |
| `resource_table` | 表リソース | キャプション、行数、列数、ヘッダ、セル、ソース範囲、説明 | `uid` | `entity_registry(uid)` | 表、IF仕様表、状態遷移表等 |
| `resource_formula` | 数式リソース | TeX数式本文、数式形式、説明 | `uid` | `entity_registry(uid)` | 計算式・条件式・制約式 |
| `resource_code` | コードリソース | コード本文、言語、コード種別、行数、説明 | `uid` | `entity_registry(uid)` | ソース、SQL、設定、IDL等 |
...
`project_artifact_setting` は、project設定画面で開発フェーズ配下に編集する成果物定義である。`dev_phase_id` で `project_dev_phase_setting.dev_phase_id` と対応し、アプリ層で同一project内の存在を検証する。設計要素ではなくプロジェクト設定であるため、`entity_registry` には登録しない。新規プロジェクトにはCORE-013の標準5フェーズ・18成果物を初期登録する。同じ成果物名（レビュー記録、障害台帳等）を複数フェーズで使用できるよう、成果物名は`project_uid, dev_phase_id, artifact_name`の組で一意とする。
...
| 5 | 開発フェーズID | `dev_phase_id` | 所属する開発フェーズ | SQLite: TEXT / App: string | No | No | 複合 | アプリ検証 | なし | 同一project内のフェーズID | 可 | 選択 | 成果物名のフェーズ単位一意性にも使用する |
| 6 | 表示順 | `sort_order` | project設定画面での表示順 | SQLite: INTEGER / App: number | No | Yes | No | なし | `0` | `>= 0` | 可 | アプリ | 並び替え用 |
| 7 | 有効フラグ | `is_active` | 設定として有効か | SQLite: INTEGER / App: boolean | No | Yes | No | なし | `1` | `0/1` | 可 | アプリ | 削除せず非表示にする場合に使う |
| 8 | 作成日時 | `created_at` | 作成日時 | SQLite: TEXT / App: ISO 8601 string | No | Yes | No | なし | `CURRENT_TIMESTAMP` | ISO 8601 | 不可 | 現在時刻 |  |
| 9 | 更新日時 | `updated_at` | 最終更新日時 | SQLite: TEXT / App: ISO 8601 string | No | Yes | No | なし | `CURRENT_TIMESTAMP` | ISO 8601 | 可 | アプリ | アプリで更新 |
...
| 8 | アーカイブフラグ | `is_archived` | Explorer上の表示対象から一時的に除外するか | SQLite: INTEGER / App: boolean | No | Yes | No | なし | `0` | `0/1` | 可 | 人/アプリ | `status` と独立。ステージ一覧には表示して復元可能にする |
| 9 | 所有者UID | `owner_uid` | 所有者、管理主体、ライフサイクル責任を持つ設計要素 | SQLite: TEXT / App: UUIDv7 string | No | No | No | `entity_registry(uid)` | NULL | UUIDv7形式 | 可 | 人/アプリ | `owns` relation_type は使わずこの属性で表す |
| 10 | レビュー情報JSON | `review_info_json` | 指摘、回答、確認結果などのレビューやり取りリスト | SQLite: TEXT / App: JSON array string | No | No | No | なし | NULL | JSON配列 | 可 | アプリ | 例: reviewer、comment、answer、status、timestamp を持つ配列 |
| 11 | メモ情報JSON | `memo_json` | 共通メモ、補足、作業メモのリスト | SQLite: TEXT / App: JSON array string | No | No | No | なし | NULL | JSON配列 | 可 | アプリ | エンティティ種別を問わず付与できるメモ |
| 12 | 管理用特記事項 | `administrative_notes` | 設計情報には使用しない管理専用情報 | SQLite: TEXT / App: string | No | No | No | なし | NULL | なし | 可 | 人手 | Markdown生成・LLM設計候補生成から除外 |
| 12 | 作成者 | `created_by` | 作成主体 | SQLite: TEXT / App: string | No | No | No | なし | なし | なし | 不可 | アプリ | user/rule/llm等 |
| 13 | 更新者 | `updated_by` | 最終更新主体 | SQLite: TEXT / App: string | No | No | No | なし | なし | なし | 可 | アプリ | 現在状態の管理用 |
| 14 | バッチ操作UID | `batch_operation_uid` | 取込・生成元のバッチ操作 | SQLite: TEXT / App: UUIDv7 string | No | No | No | `batch_operation_info(uid)` | なし | なし | 原則不可 | アプリ | 手入力作成時はNULL可 |
| 15 | ソースハッシュ | `source_hash` | 原本または入力のハッシュ | SQLite: TEXT / App: SHA-256 string | No | No | No | なし | なし | SHA-256等 | 原則不可 | アプリ | 重複検出に使う |
| 16 | 作成日時 | `created_at` | 作成日時 | SQLite: TEXT / App: ISO 8601 string | No | Yes | No | なし | `CURRENT_TIMESTAMP` | ISO 8601 | 不可 | 現在時刻 | Git履歴とは別の現在メタ |
| 17 | 更新日時 | `updated_at` | 最終更新日時 | SQLite: TEXT / App: ISO 8601 string | No | Yes | No | なし | `CURRENT_TIMESTAMP` | ISO 8601 | 可 | アプリ | ソート対象 |
...
| `resource_label` | `uid`, `label_text`, `label_kind`, `numbering`, `level`, `target_resource_uid` | ラベルリソース | 見出し、図表名、モデル名等。旧 `style_name` はUI・新規保存で使用しない |
| `resource_text` | `uid`, `text_body`, `text_role`, `language`, `target_resource_uid` | テキストリソース | 本文・説明・注記等。`target_resource_uid` は補足対象。旧 `sentences_json/context_json` はUI・新規保存で使用しない |
| `resource_list` | `uid`, `list_kind`, `item_count`, `items_json`, `max_level` | リストリソース | 物理列名は互換維持し、値はMarkdownリストとして保持・プレビューする |
| `resource_figure` | `uid`, `image_uri`, `image_hash`, `figure_number`, `caption`, `figure_kind`, `width`, `height`, `byte_size`, `image_format`, `description` | 図リソース | 画像実体はDB外。旧OCR・オブジェクト・`caption_uid`列は物理互換だけ維持し、UI・新規保存で使用しない |
| `resource_table` | `uid`, `table_title`, `row_count`, `column_count`, `table_kind`, `header_rows_json`, `header_columns_json`, `cells_json`, `source_range`, `description` | 表リソース | `table_title`はUI上「キャプション」。ヘッダ・セルJSONはスプレッドシートUIから更新する |
| `resource_formula` | `uid`, `formula_text`, `formula_format`, `formula_kind`, `description` | 数式リソース | TeX（MathJax）本文と説明。旧変数・単位・参照列はUI・新規保存で使用しない |
| `resource_code` | `uid`, `code_text`, `language`, `code_kind`, `line_count`, `description` | コードリソース | 旧シンボル・構文木・解析状態列は物理互換だけ維持し、UI・新規保存で使用しない |
...
全Resource共通の管理用特記事項は `entity_registry.administrative_notes` に保持し、設計情報、Markdown生成、LLM設計候補生成には利用しない。既存DBとの互換性のため廃止した旧列は物理的に残すが、Resource Editorの定義、保存候補、LLM入出力から除外する。図・数式から派生するResourceとの関係は `trace_link` で保持する。
...
|  5 | リンク先Resource | `target_resource_uid` | note等の補足対象 | TEXT / UUIDv7 string | No | No | No | `entity_registry(uid)` | NULL | UUIDv7形式 | 可 | 人手 | `resource://`入力可 |
...
|  4 | 項目Markdown | `items_json` | Markdownリスト記法 | TEXT / string | No | No | No | なし | NULL | なし | 可 | 抽出/編集 | Resource Editorでプレビュー |
...
| 3 | 画像ハッシュ | `image_hash` | 画像差分検出用SHA-256 | TEXT / string | No | No | No | なし | NULL | SHA-256形式 | 原則不可 | 抽出 | 抽出時に保持 |
| 4 | 図番号 | `figure_number` | 図1等の文書内番号 | TEXT / string | No | No | No | なし | NULL | なし | 可 | 抽出/人手 | 画像ハッシュ直後に表示 |
| 5 | キャプション | `caption` | 図の題名 | TEXT / Markdown | No | No | No | なし | NULL | なし | 可 | 抽出/人手 | セマンティック編集対象 |
| 6 | 図種別候補 | `figure_kind` | 構成図、処理フロー、画面図等 | TEXT / enum string | No | No | No | なし | NULL | `architecture/flow/screen/state/layout/other` | 可 | 人/LLM支援 | 候補値 |
| 7 | 幅 | `width` | 画像幅 | INTEGER / int | No | No | No | なし | NULL | `width > 0` | 原則不可 | 抽出 | px等 |
| 8 | 高さ | `height` | 画像高さ | INTEGER / int | No | No | No | なし | NULL | `height > 0` | 原則不可 | 抽出 | px等 |
| 9 | サイズ | `byte_size` | 画像ファイルのバイトサイズ | INTEGER / int | No | No | No | なし | NULL | `byte_size >= 0` | 原則不可 | 抽出 | |
| 10 | 画像フォーマット | `image_format` | PNG、JPEG等 | TEXT / string | No | No | No | なし | NULL | なし | 原則不可 | 抽出 | |
| 11 | 説明 | `description` | 文書中の図説明 | TEXT / Markdown | No | No | No | なし | NULL | なし | 可 | 人/LLM支援 | 画像をマルチモーダル入力可能 |

旧 `caption_uid`、`ocr_texts_json`、`objects_json` は既存DB互換の物理列だけを維持し、Resource Editorと新規保存では使用しない。
...
| 2 | キャプション | `table_title` | 表の題名 | TEXT / Markdown | No | No | No | なし | NULL | なし | 可 | 抽出/人手 | UI表示名は「キャプション」 |
...
| 5 | 表種別候補 | `table_kind` | IF仕様表、状態遷移表、データ項目表等 | TEXT / enum string | No | No | No | なし | NULL | `data/interface/state_transition/function_list/matrix/other` | 可 | 人/LLM支援 | |
| 6 | ヘッダ行JSON | `header_rows_json` | ヘッダ行番号 | TEXT / JSON string | No | No | No | なし | NULL | JSON形式 | 可 | アプリ | UIはヘッダ行数で編集 |
| 7 | ヘッダ列JSON | `header_columns_json` | ヘッダ列番号 | TEXT / JSON string | No | No | No | なし | NULL | JSON形式 | 可 | アプリ | UIはヘッダ列数で編集 |
| 8 | セルJSON | `cells_json` | 二次元セル内容 | TEXT / JSON string | No | No | No | なし | NULL | JSON形式 | 可 | アプリ | UIはスプレッドシート。全セルがセマンティック編集対象 |
| 9 | ソース範囲 | `source_range` | Excelセル範囲、Word表番号等 | TEXT / string | No | No | No | なし | NULL | なし | 原則不可 | 抽出 | 例: `Sheet1!A1:F20` |
| 10 | 説明 | `description` | 文書中の表説明 | TEXT / Markdown | No | No | No | なし | NULL | なし | 可 | 人/LLM支援 | 表全体をLLM入力可能 |

セル単位の安定IDは `resource_table_cell` で管理し、Resource Editorでは `cells_json.<row>.<column>` をセマンティック入力のfield nameとして構造化参照を保存する。
...
|  2 | 数式本文   | `formula_text`    | MathJax対応TeX記法           | TEXT / string        | No  | Yes | No  | なし                     | なし      | なし                                                   | 可    | 抽出/編集/LLM           |                                  |
|  3 | 数式形式   | `formula_format`  | TeX | TEXT / enum string   | No  | No  | No  | なし                     | NULL    | `latex`                     | 可    | 抽出              |                                  |
...
|  5 | 説明 | `description` | 文書中の数式説明 | TEXT / Markdown | No | No | No | なし | NULL | なし | 可 | 人/LLM支援 | 数式をLLM入力可能 |
...
| 2 | コード本文 | `code_text` | ソース、疑似コード、設定等 | TEXT / string | No | Yes | No | なし | なし | なし | 可 | 抽出/編集 | |
| 3 | 言語 | `language` | C、SQL、JSON、XML等 | TEXT / string | No | No | No | なし | NULL | なし | 可 | 抽出/人手 | |
| 4 | コード種別 | `code_kind` | ソース、疑似コード、設定等 | TEXT / enum string | No | No | No | なし | NULL | `source/pseudo/sql/config/command/idl/schema/other` | 可 | 抽出/人手 | |
| 5 | 行数 | `line_count` | 行数 | INTEGER / int | No | No | No | なし | NULL | `line_count >= 0` | 可 | アプリ | |
| 6 | 説明 | `description` | 文書中の疑似コード説明 | TEXT / Markdown | No | No | No | なし | NULL | なし | 可 | 人/LLM支援 | コード本文をLLM入力可能 |

旧 `symbols_json`、`syntax_tree_json`、`parse_status` は既存DB互換の物理列だけを維持し、Resource Editorと新規保存では使用しない。
...
| `uq_project_artifact_setting_phase_name` | `project_artifact_setting` | `project_uid, dev_phase_id, artifact_name` | UNIQUE | 同一フェーズ内の成果物名重複防止 | 初期必須 |
...
Gitコミット操作は、ユーザが明示的にステージしたファイルを維持した上で、コミット直前に xports/db_to_text/ と xports/sqlite_dump/ を全再生成・ステージする。DB to Textはテーブル毎JSONL・安定順序、SQLite dumpは schema.sql と data.sql とし、両者を同一コミットへ含める（GIT-004/007）。project.dbやblobをこの自動ステージ対象へ追加しない。
...
| 論理削除とアーカイブをどう区別するか | 通常削除は `status='deleted'` の論理削除、Explorerからの一時非表示は `is_archived=1` とする | 削除状態と、復元可能な表示整理を独立して扱える | アーカイブ中もステージ一覧とDBに保持し復元できる。物理削除はDBサイズ削減、個人情報削除、原本再取込など保守操作に限定する |
...
チャンクは成果物ごとに管理し、chunk_item により1件以上の確認済み intermediate_item と多対多で対応付ける。対応の正本トレースとして各組合せに trace_link（from_uid=chunk.uid、to_uid=intermediate_item.uid、relation_type=based_on）を作成時・更新時とも保持する。LLM候補を採用した④設計要素は、trace_link（from_uid=設計要素.uid、to_uid=生成元chunk.uid、relation_type=based_on、basis_kind=inferred、llm_run_uid設定）で生成元チャンクを追跡する。チャンク編集画面は成果物・チャンク・中間文書プレビューの3ペインとし、選択と based_on を相互強調する。チャンク固有の追加指示は chunk.additional_prompt に保持し、候補生成時に本文へ追加する。chunk.sort_order は成果物内の表示順であり、CHUNK IDを変更せずに並べ替える。新規チャンクは末尾へ追加する。章構造はチャンクへ持たせない。
...
#### 10.2.1 セマンティック入力支援データ

セマンティック入力支援では、表示文章へUID表記を埋め込まず、入力欄単位の `semantic_text`、範囲単位の `semantic_reference`、変更単位の `semantic_normalization_history` に分離する。互換マイグレーション 1.6.0 で3表と検索索引を追加する。

互換マイグレーション 1.7.0 では成果物名の一意性をフェーズ単位へ変更し、再構築時も project_artifact_relation を退避・復元して既存の文書体系を保持する。

| テーブル | 主な列 | 不変条件 |
| --- | --- | --- |
| `semantic_text` | `uid`, `project_uid`, `owner_uid`, `field_name`, `original_text`, `display_text`, `policy_json`, `created_at`, `updated_at` | `(owner_uid, field_name)` は一意。`original_text` は初回保存後に上書きせず、現在表示は `display_text` に保持する |
| `semantic_reference` | `uid`, `semantic_text_uid`, `start_offset`, `end_offset`, `surface_text`, `target_uid`, `target_kind`, `display_mode`, `relation_type`, `status`, `source`, `confidence` | 文字範囲は重複不可で `display_text` と一致し、`target_uid` は削除されていない同一プロジェクトの台帳UIDであること。`status='approved'` のときだけ検査済み `trace_link` へ確定する |
| `semantic_normalization_history` | `uid`, `semantic_text_uid`, `before_text`, `after_text`, `method`, `status`, `detail_json`, `created_at`, `decided_at` | `method` は mechanical / dictionary / llm / user、状態は candidate / approved / rejected / reverted とし、差分と取消し根拠を保持する |

`policy_json` は入力欄ごとの候補種別、許容関係種別、辞書スコープ、最小前方一致文字数、最大候補数、機械的自動正規化、既定表示方法、既定関係、強い意味関係の承認要否を保持する。未設定欄は共通既定値を利用する。候補は同一プロジェクトの `status='approved'` を基本とし、`dictionary_scope`、`version_tag`、`is_deprecated`、`access_level`を評価して、廃止・削除・権限外・異なる版の候補を確定参照にしない。

自動解析、形態素解析、LLM解析は `semantic_reference.status='candidate'` または未登録用語候補を返すだけで、辞書同義語・正規名称置換・強い設計関係を確定しない。承認済みモデル参照は `owner_uid` から `target_uid` への `trace_link` として保存し、`relation_rule_master` の起点／終点分類、必須属性、重複を同一トランザクションで検証する。自動関係の既定は `relates_to` とする。
...
| `resource_figure` | `description` | 図の説明文検索 |
...
> 実装は `backend/db/migrations.ts`（バックアップ → DDL → 版数更新）。上位桁の更新条件・DDL適用順・バックアップ手順の詳細規定は TBD-06【未適用】。
...
通常の削除操作は物理削除ではなく、`entity_registry.status='deleted'` への更新とする。Explorerから一時的に非表示にする操作は `entity_registry.is_archived=1` とし、削除状態とは独立させる。アーカイブ中の①原本・②抽出データはPipeline Navigatorの各ステージ一覧には表示し、そこから `is_archived=0` へ戻して復元できるようにする。互換マイグレーション `1.5.0` で列と検索用索引を追加する。
