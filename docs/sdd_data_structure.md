...
| 用語集 | `glossary_term` | プロジェクト固有の用語、略語、禁止語を管理する |
| 用語集 | `term_synonym` | 用語の同義語、表記揺れ、略語別表記を管理する |
...
erDiagram
    ...
    ENTITY_REGISTRY ||--o| GLOSSARY_TERM : detail
    ENTITY_REGISTRY ||--o| TERM_SYNONYM : detail
    ...
    LLM_RUN_REF ||--o{ GLOSSARY_TERM : suggested
...
| `entity_registry` | 共通台帳 | 種別、表示コード、タイトル、状態、レビュー情報、メモ情報、作成更新情報 | `uid` | `project(uid)`, `batch_operation_info(uid)` | `UNIQUE(entity_type, code)` |
...
| `intermediate_document` | 中間文書 | 中間処理ステータス、構成JSON、成果物種別ID、開発フェーズID、生成元抽出文書 | `uid` | `entity_registry(uid)`, `extracted_document(uid)` | `artifact_type_id` と `dev_phase_id` はproject設定のIDをアプリ側で検証する |
...
| `chunk` | チャンク | 中間文書参照、推定トークン数、作成日時 | `uid` | `entity_registry(uid)`, `intermediate_document(uid)` | 中間文書をLLM入力・検索用に分割した単位 |
...
| `glossary_term` | 用語 | 用語、正規化表記、定義、略語、言語、分類、禁止語フラグ | `uid` | `entity_registry(uid)`, `llm_run_ref(uid)` | 用語の状態は `entity_registry.status` で管理する |
| `term_synonym` | 用語同義語 | 用語参照、同義語・表記揺れ、種別 | `uid` | `entity_registry(uid)`, `glossary_term(uid)` | 同義語の候補/確定も `entity_registry.status` で管理する |
| `trace_link` | トレース関係 | from/to、関係種別、根拠、信頼度 | `uid` | `entity_registry(uid)` | 用語と文書・設計要素のリンクにも使う |
...
| 7 | レビュー情報JSON | `review_info_json` | 指摘、回答、確認結果などのレビューやり取りリスト | SQLite: TEXT / App: JSON array string | No | No | No | なし | NULL | JSON配列 | 可 | アプリ | 例: reviewer、comment、answer、status、timestamp を持つ配列 |
| 8 | メモ情報JSON | `memo_json` | 共通メモ、補足、作業メモのリスト | SQLite: TEXT / App: JSON array string | No | No | No | なし | NULL | JSON配列 | 可 | アプリ | エンティティ種別を問わず付与できるメモ |
| 9 | 作成者 | `created_by` | 作成主体 | SQLite: TEXT / App: string | No | No | No | なし | なし | なし | 不可 | アプリ | user/rule/llm等 |
| 10 | 更新者 | `updated_by` | 最終更新主体 | SQLite: TEXT / App: string | No | No | No | なし | なし | なし | 可 | アプリ | 現在状態の管理用 |
| 11 | バッチ操作UID | `batch_operation_uid` | 取込・生成元のバッチ操作 | SQLite: TEXT / App: UUIDv7 string | No | No | No | `batch_operation_info(uid)` | なし | なし | 原則不可 | アプリ | 手入力作成時はNULL可 |
| 12 | ソースハッシュ | `source_hash` | 原本または入力のハッシュ | SQLite: TEXT / App: SHA-256 string | No | No | No | なし | なし | SHA-256等 | 原則不可 | アプリ | 重複検出に使う |
| 13 | 作成日時 | `created_at` | 作成日時 | SQLite: TEXT / App: ISO 8601 string | No | Yes | No | なし | `CURRENT_TIMESTAMP` | ISO 8601 | 不可 | 現在時刻 | Git履歴とは別の現在メタ |
| 14 | 更新日時 | `updated_at` | 最終更新日時 | SQLite: TEXT / App: ISO 8601 string | No | Yes | No | なし | `CURRENT_TIMESTAMP` | ISO 8601 | 可 | アプリ | ソート対象 |
...
| `intermediate_document` | `uid`, `source_extracted_document_uid`, `artifact_type_id`, `dev_phase_id`, `intermediate_status`, `processor_name`, `processor_version`, `structure_json`, `settings_json`, `generated_at` | 中間文書 | 正規化、補正、分類後の文書全体構成。成果物種別と開発フェーズを関連付ける |
...
| `chunk` | `uid`, `intermediate_document_uid`, `token_count`, `created_at` | チャンク | `uid` はユーザー向けにはチャンクID。本文は持たず、中間要素との対応は `chunk_item` で管理する |
...
| `glossary_term` | `uid`, `term_text`, `normalized_text`, `definition`, `abbreviation`, `language`, `category`, `is_prohibited`, `llm_run_uid`, `confirmed_at` | 用語 | 用語状態は `entity_registry.status`。`project_id` は持たせない |
| `term_synonym` | `uid`, `term_uid`, `synonym_text`, `synonym_kind`, `created_at` | 用語同義語 | `term_uid` は `glossary_term(uid)`。候補/確定は `entity_registry.status` |
...
本格実装では `resource_list_item` を別テーブルに分ける方がよいです。
...
本格実装では `resource_table_cell` を別テーブルにするべきです。
...
| 57 | `glossary_term.term_text` | 対象 | 完全一致/部分一致/全文検索候補 | UNIQUE/FTS(MeCab) | 表示 | Yes | Yes | No | 本文 | 用語・略語検索 |
| 58 | `glossary_term.normalized_text` | 対象 | 完全一致 | UNIQUE | 詳細のみ表示 | Yes | Yes | No | フィルタ条件 | 正規化表記による重複検出 |
| 59 | `glossary_term.definition` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 詳細のみ表示 | Yes | No | No | 本文 | 定義文検索 |
| 60 | `glossary_term.is_prohibited` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | 禁止語抽出 |
| 61 | `term_synonym.term_uid` | 対象 | ID検索 | 通常/UNIQUE複合 | 詳細のみ表示 | Yes | No | No | フィルタ条件 | 用語から同義語一覧を取得 |
| 62 | `term_synonym.synonym_text` | 対象 | 完全一致/部分一致/全文検索候補 | UNIQUE/FTS(MeCab) | 表示 | Yes | Yes | No | 本文 | 同義語・表記揺れ検索 |
...
| `uq_glossary_term_normalized_language` | `glossary_term` | `normalized_text, language` | UNIQUE | 単一 `project.db` 内の用語重複防止 | 初期必須 |
| `idx_glossary_term_text` | `glossary_term` | `term_text` | 通常 | 用語検索 | 初期推奨 |
| `idx_glossary_term_category` | `glossary_term` | `category` | 通常 | 分類タグ検索 | 初期推奨 |
| `idx_glossary_term_prohibited` | `glossary_term` | `is_prohibited` | 通常 | 禁止語抽出 | 初期推奨 |
| `idx_glossary_term_llm_run` | `glossary_term` | `llm_run_uid` | 通常 | LLM候補生成元の追跡 | 後日検討 |
| `uq_term_synonym_term_text_kind` | `term_synonym` | `term_uid, synonym_text, synonym_kind` | UNIQUE | 同一用語内の同義語重複防止 | 初期必須 |
| `idx_term_synonym_term` | `term_synonym` | `term_uid` | 通常 | 用語から同義語一覧を取得 | 初期必須 |
| `idx_term_synonym_text` | `term_synonym` | `synonym_text` | 通常 | 同義語・表記揺れ検索 | 初期推奨 |
...
CREATE TABLE term_synonym (
    ...
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (term_uid, synonym_text, synonym_kind),
    FOREIGN KEY (uid) REFERENCES entity_registry(uid) ON DELETE CASCADE,
    FOREIGN KEY (term_uid) REFERENCES glossary_term(uid) ON DELETE CASCADE
);
...
CREATE INDEX idx_glossary_term_text ON glossary_term(term_text);
CREATE INDEX idx_glossary_term_category ON glossary_term(category);
CREATE INDEX idx_glossary_term_prohibited ON glossary_term(is_prohibited);
CREATE INDEX idx_glossary_term_llm_run ON glossary_term(llm_run_uid);
CREATE INDEX idx_term_synonym_term ON term_synonym(term_uid);
CREATE INDEX idx_term_synonym_text ON term_synonym(synonym_text);
...
| `project_artifact_setting` | `project_artifact_setting` | `ART` | プロジェクト成果物設定 | project設定画面で管理する成果物設定 |
| `project_artifact_relation` | `project_artifact_relation` | `ARTREL` | 成果物文書体系 | 成果物設定同士の親子関係を管理する |
| `project_dev_phase_setting` | `project_dev_phase_setting` | `PHASE` | プロジェクト開発フェーズ設定 | project設定画面で管理する開発フェーズ設定 |
...
| `glossary_term` | `glossary_term` | `TERM` | 用語 | プロジェクト固有の用語、略語、禁止語 |
| `term_synonym` | `term_synonym` | `SYN` | 用語同義語 | 同義語、表記揺れ、略語別表記 |
| `trace_link` | `trace_link` | `TRACE` | トレース関係 | 関係自体も台帳登録する。用語と設計要素の対応にも使う |
...
`project_artifact_setting`、`project_artifact_relation`、`project_dev_phase_setting` は project設定情報であるが、`entity_type` のCHECK許容値には物理テーブル名として含める。DB to Textではテーブル毎JSONLとして出力する。

将来、新しい管理対象テーブルを追加する場合は、`entity_type` のCHECK制約、code prefix、DB to Text出力対象、FTS対象有無を同じマイグレーションで追加する。

### 10.2 用語集設計

`glossary_term` は、プロジェクト固有の用語、略語、禁止語を管理する。用語は設計要素と同じく `entity_registry` に登録し、状態は `entity_registry.status` で管理する。候補用語は `draft` または `review`、確定用語は `approved`、棄却用語は `rejected`、削除済み用語は `deleted` として扱う。

`term_synonym` は、用語の同義語、表記揺れ、略語別表記を管理する。候補/確定の状態は `term_synonym` の独自列ではなく、同じく `entity_registry.status` で管理する。

用語と文書・設計要素とのリンクは `trace_link` を使う。`from_uid` または `to_uid` に `glossary_term.uid` を指定し、相手側に `resource_text`、`resource_table`、`resource_model` などの設計リソースUIDを指定する。
...
| `glossary_term` | `term_text`, `normalized_text`, `definition`, `abbreviation`, `category` | 用語・定義検索 |
| `term_synonym` | `synonym_text` | 同義語・表記揺れ検索 |
