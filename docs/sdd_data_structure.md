...
| 中間データ | `intermediate_document` | 抽出結果を正規化・補正した中間文書全体の構成情報をJSONで保持する |
...
| `entity_registry` | 共通台帳 | 種別、表示コード、タイトル、状態、作成更新情報 | `uid` | `project(uid)`, `batch_operation_info(uid)` | `UNIQUE(entity_type, code)` |
...
| `intermediate_document` | 中間文書 | 中間処理ステータス、構成JSON、生成元抽出文書 | `uid` | `entity_registry(uid)`, `extracted_document(uid)` | 抽出結果を正規化・補正した中間文書全体 |
...
| 9 | バッチ操作UID | `batch_operation_uid` | 取込・生成元のバッチ操作 | SQLite: TEXT / App: UUIDv7 string | No | No | No | `batch_operation_info(uid)` | なし | なし | 原則不可 | アプリ | 手入力作成時はNULL可 |
...
| `intermediate_document` | `uid`, `source_extracted_document_uid`, `intermediate_status`, `processor_name`, `processor_version`, `structure_json`, `settings_json`, `generated_at` | 中間文書 | 正規化、補正、分類後の文書全体構成 |
...
| 8 | `project_artifact_setting.artifact_name` | 対象 | 完全一致/部分一致 | UNIQUE/複合 | 表示 | Yes | Yes | No | メタ情報 | project設定画面の成果物検索 |
| 9 | `project_artifact_setting.artifact_type_id` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | 成果物種別フィルタ |
| 10 | `project_artifact_relation.parent_artifact_uid` | 対象 | ID検索 | 複合 | 詳細のみ表示 | Yes | Yes | No | フィルタ条件 | 文書体系ツリーの親指定 |
| 11 | `project_artifact_relation.child_artifact_uid` | 対象 | ID検索 | 複合 | 詳細のみ表示 | Yes | No | No | フィルタ条件 | 子成果物から親を逆引きする |
| 12 | `project_dev_phase_setting.dev_phase_id` | 対象 | 完全一致 | UNIQUE/複合 | 表示 | Yes | Yes | No | フィルタ条件 | 開発フェーズID検索 |
| 13 | `project_dev_phase_setting.dev_phase_name` | 対象 | 完全一致/部分一致 | 通常 | 表示 | Yes | Yes | No | メタ情報 | 開発フェーズ名検索 |
| 14 | `source_document.file_name` | 対象 | 部分一致 | 通常 | 表示 | Yes | Yes | No | 根拠 | 原本検索 |
| 15 | `source_document.file_type` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 |  |
| 16 | `source_document.file_hash` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | メタ情報 | 同一性確認 |
| 17 | `source_location.source_document_uid` | 対象 | 完全一致 | 通常 | 非表示 | Yes | No | No | フィルタ条件 | 原本内位置一覧 |
| 18 | `extracted_document.source_document_uid` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 原本から抽出文書を逆引き |
| 19 | `extracted_document.extraction_status` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | 抽出状態フィルタ |
| 20 | `source_location.section_path` | 対象 | 前方一致/部分一致 | 通常 | 表示 | Yes | Yes | No | 将来候補 | 章節根拠。RAGは初期導入しない |
| 21 | `source_location.page_no_start` | 対象 | 範囲検索 | 通常 | 表示 | Yes | Yes | No | 根拠 | PDF/Word向け |
| 22 | `extracted_item.extracted_document_uid` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 抽出文書内の要素一覧 |
| 23 | `extracted_item.item_type` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | `resource_*` テーブル名 |
| 24 | `extracted_item.source_document_uid` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 原本別検索 |
| 25 | `extracted_item.source_location_uid` | 対象 | ID検索 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 原本内位置から抽出要素を逆引き |
| 26 | `extracted_item.resource_uid` | 対象 | ID検索 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 対応する設計リソース。トレース探索ではこのUIDを使う |
| 27 | `intermediate_document.source_extracted_document_uid` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 抽出文書から中間文書を逆引き |
| 28 | `intermediate_document.intermediate_status` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | 中間処理状態フィルタ |
| 29 | `intermediate_item.intermediate_document_uid` | 対象 | 完全一致 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 中間文書内の要素一覧 |
| 30 | `intermediate_item.item_type` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | `resource_*` テーブル名 |
| 31 | `intermediate_item.resource_uid` | 対象 | ID検索 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 対応する設計リソース。トレース探索ではこのUIDを使う |
| 32 | `resource_label.label_text` | 対象 | 完全一致/部分一致/全文検索候補 | FTS(MeCab) | 表示 | Yes | Yes | No | 本文 | 見出し、図表名、モデル名等 |
| 33 | `resource_label.label_kind` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | 文書名、章、節、図名、表名等 |
| 34 | `resource_text.text_body` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 表示 | Yes | No | No | 本文 | 本文検索の主要対象 |
| 35 | `resource_text.text_role` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | body/note/comment等 |
| 36 | `resource_table.table_title` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 表示 | Yes | Yes | No | メタ情報 | 表名検索 |
| 37 | `resource_table.table_kind` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | IF仕様表、状態遷移表等 |
| 38 | `resource_figure.figure_kind` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | 構成図、画面図等 |
| 39 | `resource_formula.formula_text` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 表示 | Yes | No | No | 本文 | 数式・条件式検索 |
| 40 | `resource_code.code_text` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 表示 | Yes | No | No | 本文 | ソース、SQL、設定検索 |
| 41 | `resource_model.model_name` | 対象 | 部分一致 | 通常 | 表示 | Yes | Yes | No | メタ情報 | モデル名検索 |
| 42 | `resource_scenario.scenario_name` | 対象 | 部分一致 | 通常 | 表示 | Yes | Yes | No | メタ情報 | シナリオ名検索 |
| 43 | `resource_interface.interface_kind` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | API、通信、DB、画面等 |
| 44 | `resource_state_transition.state_machine_name` | 対象 | 部分一致 | 通常 | 表示 | Yes | Yes | No | メタ情報 | 状態機械名検索 |
| 45 | `resource_data_structure.data_structure_kind` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | DB表、メッセージ等 |
| 46 | `resource_reference.reference_text` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 表示 | Yes | No | No | 根拠 | 参照表現検索 |
| 47 | `resource_reference.resolution_status` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 | 未解決参照の抽出 |
| 48 | `resource_metadata.metadata_key` | 対象 | 完全一致/前方一致 | 複合 | 詳細のみ表示 | Yes | Yes | No | メタ情報 | メタデータキー検索 |
| 49 | `trace_link.from_uid` | 対象 | ID検索 | 通常/複合 | 詳細のみ表示 | Yes | No | No | フィルタ条件 | 前方探索 |
| 50 | `trace_link.to_uid` | 対象 | ID検索 | 通常/複合 | 詳細のみ表示 | Yes | No | No | フィルタ条件 | 逆引き |
| 51 | `trace_link.relation_type` | 対象 | 完全一致 | 通常/複合 | 表示 | Yes | Yes | No | フィルタ条件 | 関係種別 |
| 52 | `trace_link.rationale` | 対象 | 部分一致/全文検索候補 | FTS(MeCab) | 詳細のみ表示 | Yes | No | No | 将来候補 | RAGは初期導入しない |
| 53 | `llm_run_ref.input_ref_uid` | 対象 | ID検索 | 通常 | 詳細のみ表示 | Yes | No | No | 根拠 | 入力追跡 |
| 54 | `llm_run_ref.status` | 対象 | 完全一致 | 通常 | 表示 | Yes | Yes | No | フィルタ条件 |  |
...
| `intermediate_item` | `text_content` | 中間本文検索 |
