...
`structure_json` はPowerPoint原本のスライド構造と座標付き抽出結果を保持する正本であり、レビュー用Markdown、LLM入力用Markdown、構造JSON、画像アセット、ZIPダウンロード相当ファイルは派生成果物として扱う。D2Dでは `structure_json`、`resource_*`、`source_location`、`blob_resource` を正本とし、ZIP相当出力は `blobs/exports/` または `exports/` の派生成果物に分類する。
