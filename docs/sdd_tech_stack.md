...
| グラフ描画 | SVG 自前実装 | — | — | 関係グラフ・影響分析の可視化。力学レイアウト、階層表示、ホップ距離による強調・減衰表示を SVG + React で自前実装。PoCのVis.js NetworkのUXは参考にするが、Vis.jsは**未導入**（Cytoscape.js 等も将来移行候補） |
...
### 5.5 仕様書・設計書要素抽出PoCからの採用判断

> **採用判断**: `ref/support_srs_260607` の考え方のうち、正規化テキスト生成、設計要素候補、関係候補、保存前の表形式編集、要素名変更時の関係候補追従、関係グラフのフィルタ・レイアウト切替・ホップ強調、AI通信ログは採用する。ただし、PoCの Express API、`entities` / `relations` 専用SQLite、Vis.js Network、FontAwesome、`@google/generative-ai` SDK、ブラウザカード型UIはそのまま採用しない。D2Dでは Local Backend の操作単位API、`entity_registry` / `resource_*` / `trace_link`、SVG + React のTrace Graph、Serendie Symbols、fetchベースLLMプロバイダ、Workbench型UXへ置き換える。

## 6. LLM クライアント（Local Backend）

すべての Provider は Local Backend の `globalThis.fetch` を用いた HTTP リクエストで実装する。外部 SDK（openai npm、@google/generative-ai 等）は**未導入**。初期設定画面は OpenAI / Gemini / Azure OpenAI を対象に含める。
