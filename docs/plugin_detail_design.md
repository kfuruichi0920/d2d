...
本ツール側の共通プラグインソケット機能は、プラグインプラグを動かすための最小限のホスト機能に絞る。設定、権限、外部送信可否、manifest検証はプラグイン管理に含め、個別の共通機能として細分化しない。
...
トレースマトリクス、関係探索、影響分析を扱う場合は、設計編集構成に以下を追加する。
