...
test('②→③統合・編集・確定（P7）', async () => {
  ...
  const structureOutline = page.getByTestId('intermediate-structure-outline')
  await expect(structureOutline.getByRole('treeitem').first()).toBeVisible()
  await structureOutline.getByRole('treeitem').nth(1).click()
...
test('編集機能: 用語集・状態遷移・表編集・検証編集（P10）', async () => {
  ...
  await page.getByTestId('intermediate-preview-structure').click()
  await page.getByTestId('intermediate-outline-i7').click()
  await page.getByTestId('intermediate-preview-visual').click()
