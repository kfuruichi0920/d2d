...
test('②→③統合・編集・確定（P7）', async () => {
  ...
  const semanticEditor = page.getByTestId('code-editor')
  ...
  const standaloneSemanticEditor = page.getByTestId('code-editor')
  ...
  const labelSemanticEditor = page.getByTestId('code-editor')
...
test('編集機能: 用語集・状態遷移・表編集・検証編集（P10）', async () => {
  ...
  await expect(semanticInput.getByTestId('semantic-integrate-text_body')).toBeVisible()
