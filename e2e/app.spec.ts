...
test('プロジェクト作成でタイトル・パイプラインが更新される（P1〜P3 連携）', async () => {
  ...
  await expect(page.getByTestId('title-schema-version')).toHaveText('schema 2.3.0')
...
test('Excel原本→候補生成・編集→範囲限定LLM確認→②抽出の全経路（P5-19）', async () => {
  ...
  await expect
    ...
    .toBe('generated')
  await expect(page.getByTestId('extract-button')).toHaveText('抽出グループ候補を確認')
  await page.getByTestId('extract-button').click()

  await expect(page.getByTestId('excel-extraction-editor')).toBeVisible()
  await expect(page.getByTestId('excel-sheet-要求')).toBeVisible()
  ...
  await page.getByTestId('excel-candidate-title').fill('要求一覧（確認済）')
  await page.getByTestId('excel-candidate-status').selectOption('approved')
  await page.getByTestId('excel-candidate-save').click()
  await expect(page.getByTestId('notifications')).toContainText('Excel抽出グループ候補を保存しました')

  await page.getByTestId('excel-candidate-llm').click()
  await expect(page.getByTestId('llm-request-dialog')).toBeVisible()
  await page.getByTestId('llm-preview-button').click()
  ...
  await expect(page.getByTestId('element-grid')).toContainText('要求一覧（確認済）')
