...
test('Workbenchパネルのリサイズ・アコーディオン・再帰分割（P3-1）', async () => {
  ...
  await expect(page.getByTestId('secondary-accordion-evidence')).toBeVisible()
  await page.getByTestId('secondary-tab-evidence').click()
  await expect(page.getByTestId('secondary-tab-evidence')).toHaveAttribute('aria-expanded', 'false')
  await page.getByTestId('secondary-tab-evidence').click()
  await expect(page.getByTestId('secondary-tab-evidence')).toHaveAttribute('aria-expanded', 'true')
