...
test('Excel原本→候補生成・編集→範囲限定LLM確認→②抽出の全経路（P5-19）', async () => {
  ...
  await expect(page.getByTestId('excel-extraction-editor')).toContainText('未確認 2')
  ...
  await expect(page.getByTestId('excel-candidate-text-preview')).toContainText('温度異常時に停止する')
