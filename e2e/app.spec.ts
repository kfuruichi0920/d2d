...
async function closeAllEditorTabs(): Promise<void> {
  for (let group = 0; group < 10; group += 1) {
    ...
    await page.getByTestId('tab-menu-close-all').click()
    await expect.poll(async () => tabs.count()).toBeLessThan(before)
  }
  throw new Error('Editorタブをすべて閉じられませんでした')
}
...
test('Workbench シェルが表示される（P3-1）', async () => {
  ...
  // ①〜④・分析の流れをアニメーション付きSVG図解で表示する
  await expect(page.getByTestId('help-workflow-diagram')).toBeVisible()
  await expect(page.getByTestId('help-workflow-diagram')).toContainText('設計モデル')
  ...
  // entity_registryを中心としたハブ図解
  await expect(page.getByTestId('help-schema-diagram')).toBeVisible()
  await expect(page.getByTestId('help-schema-diagram')).toContainText('resource_*')
  await page.getByTestId('help-schema').getByRole('button', { name: '設計モデル' }).click()
  await expect(page.getByTestId('help-design-model')).toContainText('プロジェクトを開くと')
...
test('作業モード切替後もWorkbench外周パネル状態を維持する（P3-7 / UI-041）', async () => {
  ...
  await page.keyboard.press('Control+Shift+P')
  await page.getByTestId('palette-input').fill('作業モード: M3')
  await page.keyboard.press('Enter')
  await expect(page.getByTestId('status-mode')).toHaveCount(0)
  ...
  await page.keyboard.press('Control+Shift+P')
  await page.getByTestId('palette-input').fill('作業モード: M0')
  await page.keyboard.press('Enter')
  await expect(page.getByTestId('status-mode')).toHaveCount(0)
  await expect(page.getByTestId('primary-sidebar')).toBeVisible()
  await expect(page.getByTestId('secondary-sidebar')).toBeVisible()
...
test('プロジェクト作成でタイトル・パイプラインが更新される（P1〜P3 連携）', async () => {
  ...
  // project.opened イベント → Title Barの製品／schema版数、Status Bar、パイプライン件数へ反映。
  ...
  await expect(page.getByTestId('title-schema-version')).toHaveText('schema 2.5.0')
  ...
  await expect(page.getByTestId('status-llm-provider')).toHaveText('LLM Provider: ollama 設定済')
  await expect(page.getByTestId('status-llm-external')).toHaveText('外部LLM: 対象外')
  ...
  for (const statusId of [
    ...
    await expect(page.getByTestId(statusId).locator('svg')).toHaveCount(1)
  }
  // 実行環境設定は責務に応じたSettings Resourceを通常タブで開く（UI-009）。
  for (const statusId of ['status-plantuml', 'status-mecab', 'status-llm-provider']) {
    await page.getByTestId(statusId).click()
    await expect(page.getByTestId('settings-editor')).toBeVisible()
  }
  for (const statusId of ['status-llm-external', 'status-debug-level']) {
    await page.getByTestId(statusId).click()
  ...
  // Pipelineの分析は②抽出・③中間・チャンク・④設計の全Resource集合、用語集は専用Editorを開く。
  await page.getByTestId('pipeline-analysis').click()
  await expect(page.getByTestId('trace-impact')).toBeVisible()
  await expect(page.getByTestId('trace-impact').locator('.trace-impact-column')).toHaveCount(4)
  await expect(page.getByTestId('impact-scopes-0')).toHaveValue('all:extracted')
  await expect(page.getByTestId('impact-scopes-1')).toHaveValue('all:intermediate')
  await expect(page.getByTestId('impact-scopes-2')).toHaveValue('all:chunk')
  await expect(page.getByTestId('impact-scopes-3')).toHaveValue('all:design')
  ...
  // Alt+Dでアドレスバーへ移動し、F5は入力フォーカス中でもアクティブEditorを更新する（UI-058）。
...
test('オントロジー設定を保存し、Helpへ動的反映する（MODEL-019〜029 / UI-060〜061）', async () => {
  // 設計モデルHelpの概念図（SW設計オントロジー）を、関係を無効化する前の状態で確認する
  const address0 = page.getByTestId('resource-address')
  await address0.fill('help://design-model')
  await address0.press('Enter')
  await expect(page.getByTestId('help-ontology-diagram')).toBeVisible()
  await expect(page.getByTestId('help-ontology-diagram')).toContainText('要求')
  await expect(page.getByTestId('help-ontology-diagram')).toContainText('model_req')
  await expect(page.getByTestId('help-ontology-diagram')).toContainText('割当')
  const legend = page.getByTestId('help-ontology-legend')
  await expect(legend).toBeVisible()
  const allocatedChip = legend.locator('.d2d-legend-chip', { hasText: '割当' })
  await expect(allocatedChip).toBeVisible()
  // 凡例チップへのhoverで強調表示のクラスが切り替わる（動的な図解）
  await allocatedChip.hover()
  await expect(allocatedChip).toHaveClass(/active/)
  await page.keyboard.press('Control+W')

  await page.getByTestId('activity-settings').click()
  await page.getByTestId('open-design-model-settings').click()
  await expect(page.getByTestId('design-model-settings-editor')).toContainText('ツール設定 └ 設計モデル設定')
  const settings = page.getByTestId('ontology-settings')
  await expect(settings).toBeVisible()
  await expect(settings).toContainText('v0.1.0')
  await expect(settings.getByTestId('ontology-setting-guide')).toContainText('設定方法')
  await settings.getByTestId('add-attribute-model_req 属性定義').click()
  const addedAttribute = settings.getByLabel('model_req 属性定義 表示名 4')
  await addedAttribute.fill('E2E追加属性')
  await addedAttribute.locator('xpath=..').getByRole('button', { name: '無効化' }).click()

  const definition = settings.getByLabel('model_req 日本語定義')
  await definition.fill('E2Eで更新した要求モデル定義。')
  await definition.locator('xpath=..').getByRole('button', { name: '保存' }).click()
  await expect(page.getByTestId('notifications')).toContainText('model_req の定義を保存しました')

  await settings.getByLabel('allocated_to アイコン背景色').evaluate((element) => {
    ;(element as HTMLInputElement).value = '#123456'
    element.dispatchEvent(new Event('input', { bubbles: true }))
  })
  await settings.getByLabel('allocated_to アイコン文字').fill('Use')
  const usesRow = settings.getByLabel('allocated_to アイコン文字').locator('xpath=..')
  await usesRow.getByRole('checkbox', { name: '有効' }).uncheck()
  await usesRow.getByRole('button', { name: '保存' }).click()
  await expect(page.getByTestId('notifications')).toContainText('allocated_to の定義を保存しました')

  await settings.getByLabel('関係種別').selectOption('satisfies')
  await expect(settings.getByLabel('satisfies model_func to model_req')).toBeChecked()
  await settings.getByRole('button', { name: '設定を確定して版更新' }).click()
  await expect(settings).toContainText('v0.1.1')

  const address = page.getByTestId('resource-address')
  await address.fill('help://design-model')
  await address.press('Enter')
  await expect(page.getByTestId('help-design-model')).toContainText('E2Eで更新した要求モデル定義。')
  await expect(page.getByTestId('help-design-model')).toContainText('v0.1.1')
})
...
test('Workbenchパネルのリサイズ・アコーディオン・再帰分割（P3-1）', async () => {
  ...
  await expect(page.getByTestId('editor-group-3')).toHaveClass(/is-active-group/)
...
test('ジョブ実行が Jobs Panel と Status Bar に反映される（P3-5）', async () => {
  ...
  // ジョブ一覧には作成時刻等が表示される（CORE-024）
  await expect(page.getByTestId('jobs-list').locator('[data-testid^="job-time-"]').first()).toBeVisible()
...
test('③→④候補生成→候補レビュー→採用の全経路（P8）', async () => {
  ...
  const candidateJson = JSON.stringify({
    elements: [
      { temp_id: 't1', category: 'model_req', title: '応答時間要求', description: '100ms以内', evidence: '100ms以内' },
      { temp_id: 't2', category: 'model_func', title: '応答処理機能' },
      { temp_id: 't3', category: 'model_struct', title: '利用コンポーネント' },
      { temp_id: 't4', category: 'model_data', title: '利用データ' }
    ],
    relations: [
      { from_temp_id: 't2', to_temp_id: 't1', relation_type: 'satisfies', rationale: '機能が要求を満たす' },
      { from_temp_id: 't3', to_temp_id: 't4', relation_type: 'uses', rationale: 'データを利用する' }
    ],
    warnings: []
  })
  const mock = createServer((req, res) => {
  ...
  try {
    ...
    await expect(page.getByTestId('element-title-req-01')).toHaveValue('応答時間要求')
    await expect(page.getByTestId('relation-row-0')).toContainText('req-01: 応答時間要求')
    await expect(page.getByTestId('candidate-relation-attribute-1')).toHaveValue('')
    await expect(page.getByTestId('candidate-relation-attribute-1')).toContainText('仮設定（作成中）')
    await page.getByTestId('element-title-req-01').fill('応答時間要求（改）')
    await expect(page.getByTestId('relation-row-0')).toContainText('req-01: 応答時間要求（改）')

    // 他タブとの往復ではRenderer内の編集状態を保持する。
    await page.getByTestId('stage-intermediate').click()
    await page.getByRole('tab', { name: /④候補セット/ }).click()
    await expect(page.getByTestId('element-title-req-01')).toHaveValue('応答時間要求（改）')

    // 明示的一時保存はタブを閉じても1セット再開でき、開き直し直後は原本を表示する。
    await page.getByTestId('candidate-save-draft').click()
    const candidateUri = await page.getByTestId('resource-address').inputValue()
    await page.keyboard.press('Control+W')
    await page.getByTestId('resource-address').fill(candidateUri)
    await page.getByTestId('resource-address').press('Enter')
    await expect(page.getByTestId('element-title-req-01')).toHaveValue('応答時間要求')
    await page.getByTestId('candidate-resume-draft').click()
    await expect(page.getByTestId('element-title-req-01')).toHaveValue('応答時間要求（改）')
    const categorySelect = page.getByTestId('candidate-elements').locator('tbody tr').nth(1).locator('select')
    await categorySelect.selectOption('model_req')
    ...
    await categorySelect.selectOption('model_func')
    ...
    const adoptedRelations = await page.evaluate(async () => {
      const [uses, basedOn] = await Promise.all([
        window.api.invoke('design.listRelations', { relationType: 'uses' }),
        window.api.invoke('design.listRelations', { relationType: 'based_on' })
      ])
      return { uses, basedOn }
    })
    expect(adoptedRelations.uses).toMatchObject({ ok: true })
    expect((adoptedRelations.uses as { ok: true; result: Array<{ review_status: string }> }).result).toContainEqual(
      expect.objectContaining({ review_status: 'creating' })
    )
    expect(
      (adoptedRelations.basedOn as { ok: true; result: Array<{ to_code: string }> }).result.some((link) =>
        link.to_code.startsWith('CHUNK-')
      )
    ).toBe(true)
    await expect(page.getByTestId('design-tree')).toHaveAttribute('open', '')
    await expect(page.getByTestId('design-el-REQ-000001')).toHaveAttribute('title', /model_req/)
    ...
    await expect(page.getByTestId('design-el-REQ-000001').locator('.d2d-explorer-tags')).toContainText('要求')
    ...
    await expect(page.getByTestId('add-state-machine')).toHaveCount(0)
    await expect(page.getByTestId('open-glossary')).toHaveCount(0)
    await expect(page.getByTestId('open-model-editor')).toHaveText('+PlantUML')

    // 右クリックから他モデルとの関係登録ダイアログを開く。
    await page.getByTestId('stage-design-row-REQ-000001').click({ button: 'right' })
    await expect(page.getByTestId('design-relation-dialog')).toBeVisible()
    // 後続の関係マトリクスで relates_to をトグル検証するため、独立した種別を登録する。
    await page.getByTestId('relation-type').selectOption('conflicts_with')
    await page.getByTestId('relation-required-attribute').selectOption('suspected')
    await page.getByTestId('relation-save').click()
    await expect(page.getByTestId('design-relation-dialog')).toHaveCount(0)
    await page.getByTestId('sort-model_type').click()
    await expect(page.getByTestId('sort-model_type')).toContainText('▲')
    ...
    const funcRow = page.getByTestId('stage-design-row-FUNC-000001')
    await funcRow.click()
    await expect(page.getByTestId('design-element-viewer')).toHaveCount(0)
    await funcRow.press('Enter')
    ...
    await expect(page.getByTestId('design-element-viewer')).toContainText('REQ-000001')

    // candidate://一覧（LLM-015）: ID・名称・種別に加え、作成時刻・ステータスを表示する
    const addressBar = page.getByTestId('resource-address')
    await addressBar.fill('candidate://')
    await addressBar.press('Enter')
    await expect(page.getByTestId('address-list-candidate')).toBeVisible()
    const candidateRow = page.getByTestId('address-list-candidate').locator('tbody tr').first()
    await expect(candidateRow).toContainText(/\d{4}\/\d{1,2}\/\d{1,2}/)
    await expect(candidateRow.locator('[data-testid^="address-entry-status-"]')).toContainText('success')
  } finally {
    mock.close()
  }
})
...
test('トレースクエリ→グラフ→マトリクス→整合性検査（P9）', async () => {
  // 前工程のタブ折返しでマトリクスの縦領域を圧迫しないよう、トレース試験は空のEditor Areaから開始する。
  ...
  await page.evaluate(async () => {
    await window.api.invoke('design.createElement', { modelType: 'model_req', title: 'マトリクス追加要求' })
    await window.api.invoke('design.createElement', { modelType: 'model_func', title: 'マトリクス追加機能' })
  })
  await page.getByTestId('open-matrix').click()
  await page.getByTestId('open-matrix').click()
  await expect(page.locator('.wb-tab-title', { hasText: 'トレースマトリクス' })).toHaveCount(2)
  await page.getByRole('button', { name: 'トレースマトリクス を閉じる' }).first().click()
  await expect(page.locator('.wb-tab-title', { hasText: 'トレースマトリクス' })).toHaveCount(1)
  const traceMatrix = page.getByTestId('trace-matrix')
  await expect(traceMatrix).toBeVisible()
  await expect(page.getByTestId('trace-matrix-row-scopes')).toHaveValue('all:design')
  await expect(page.getByTestId('trace-matrix-col-scopes')).toHaveValue('all:design')
  ...
  // 関係種別は全選択ON/OFFでき、OFF時はリンク表示も消える。
  const allRelations = page.getByTestId('trace-relation-all')
  await allRelations.check()
  await expect(page.getByTestId('trace-relation-based_on')).toBeChecked()
  await expect(page.getByTestId('trace-relation-satisfies')).toBeChecked()
  await allRelations.uncheck()
  await expect(page.getByTestId('trace-relation-based_on')).not.toBeChecked()
  await expect(page.getByTestId('trace-relation-satisfies')).not.toBeChecked()
  await expect(traceMatrix.locator('td.has-relation')).toHaveCount(0)
  ...
  const firstResourceBasedOnCell = traceMatrix.locator('td.has-relation').first()
  const resourceBasedOnTestId = await firstResourceBasedOnCell.getAttribute('data-testid')
  expect(resourceBasedOnTestId).toBeTruthy()
  const resourceBasedOnCell = page.getByTestId(resourceBasedOnTestId!)
  await expect(resourceBasedOnCell).toContainText('→B')
  await resourceBasedOnCell.dispatchEvent('click')
  await expect(resourceBasedOnCell).not.toContainText('→B')
  await resourceBasedOnCell.dispatchEvent('click')
  await expect(resourceBasedOnCell).toContainText('→B')
  ...
  await rowScopes.selectOption('design:model_func')
  await colScopes.selectOption('design:model_req')
  await page.getByTestId('trace-relation-based_on').uncheck()
  await page.getByTestId('trace-relation-relates_to').check()
  await page.getByTestId('trace-relation-satisfies').check()
  const usesOption = page.getByTestId('trace-relation-allocated_to').locator('xpath=..')
  await expect(usesOption).toContainText('Use')
  await expect(usesOption).toContainText('無効')
  await page.getByTestId('trace-relation-allocated_to').check()
  await expect(page.getByTestId('trace-relation-attribute-allocated_to')).toContainText('仮設定（作成中）')
  await page.getByTestId('trace-relation-allocated_to').uncheck()
  await expect(page.getByTestId('trace-relation-satisfies')).toBeChecked()
  ...
  await firstMatrixCell.dispatchEvent('click')
  ...
  await page.getByTestId('trace-row-FUNC-000001').dispatchEvent('click')
  await expect(traceMatrix).toContainText('2 セル選択')
  await expect(page.getByTestId('trace-matrix-add')).toHaveText('選択')
  await expect(page.getByTestId('trace-matrix-delete')).toHaveText('削除')
  ...
  await page.getByTestId('trace-col-REQ-000001').dispatchEvent('click')
  await expect(traceMatrix).toContainText('2 セル選択')
  await page.getByTestId('trace-col-REQ-000002').dispatchEvent('click', { ctrlKey: true })
...
test('編集機能: 用語集・状態遷移・表編集・検証編集（P10）', async () => {
  ...
  await page.getByTestId('stage-design').click()
  await page.getByLabel('追加する設計モデル種別').selectOption('model_state')
  await page.getByLabel('新しい設計モデルの名称').fill('新しい状態機械')
  await page.getByTestId('add-design-model').click()
  await expect(page.getByTestId('state-machine-editor')).toHaveCount(0)
  await page.getByTestId('stage-design-row-STATE-000001').dblclick()
  ...
  await expect(page.getByTestId('design-element-viewer')).toBeVisible()
  await page.getByTestId('design-field-condition').fill('通常負荷時')
  await page.getByTestId('design-field-expected_result').fill('100ms以内に応答する')
  await page.getByTestId('design-model-save').click()
...
test('レポート生成→Markdown/HTMLプレビュー（P13）', async () => {
  ...
  await page.getByTestId('report-cat-model_req').check()
  await page.getByTestId('report-generate').click()
...
test('ショートカットキーのカスタマイズ（W1 / UI-023）', async () => {
  ...
  await page.getByTestId('keybinding-filter').fill('アドレスバーへ移動')
  await expect(page.getByTestId('keybinding-value-nav.focusAddress')).toHaveText('Alt+D')
  await page.getByTestId('keybinding-filter').fill('アクティブEditor Group')
  await expect(page.getByTestId('keybinding-row-editor.group.activate.1')).toBeVisible()
  await expect(page.getByTestId('keybinding-value-editor.group.activate.1')).toHaveText('Ctrl+1')
  await expect(page.getByTestId('keybinding-row-editor.group.activate.9')).toBeVisible()
  await expect(page.getByTestId('keybinding-value-editor.group.activate.9')).toHaveText('Ctrl+9')
...
test('ユーザ操作のUndo/Redo（W4 / NFR-012）', async () => {
  ...
  await page.keyboard.press('Control+Z')
  await expect(page.getByTestId('notifications')).toContainText('元に戻しました')
  await expect(page.getByTestId('stage-source-row-DOC-000002')).toBeVisible({ timeout: 15_000 })
})
...
test('Undo拡張: ③削除の復元とマトリクス関係の取り消し（W7 / NFR-012）', async () => {
  ...
  await page.getByTestId('trace-matrix-row-scopes').selectOption('design:model_func')
  await page.getByTestId('trace-matrix-col-scopes').selectOption('design:model_req')
  await page.getByTestId('trace-relation-based_on').uncheck()
  await page.getByTestId('trace-relation-relates_to').check()
...
test('LLMログの生送受信表示と候補再作成（W12、LLM-015）', async () => {
  await page.getByTestId('status-jobs').click()
  await page.getByTestId('panel-tab-llm').click()
  await expect(page.getByTestId('llm-logs-list')).toBeVisible()
  // ログ一覧には時刻・問い合わせ種別（process_name）が表示される
  const designCandidateRow = page
    .getByTestId('llm-logs-list')
    .locator('.d2d-list-row', { hasText: 'design-candidates' })
    .first()
  await expect(designCandidateRow).toContainText(/\d{4}\/\d{1,2}\/\d{1,2}/)
  await expect(designCandidateRow).toContainText('design-candidates')

  // ログ一覧から直接「候補を開く」で、LLMを再実行せず保存済み応答から候補セットを再構成できる
  await designCandidateRow.getByTestId(/llm-log-open-candidate-/).click()
  await expect(page.getByTestId('candidate-editor')).toBeVisible()
  await page.keyboard.press('Control+W')

  // design-candidates（P8で生成済み）の実行詳細を開く
  await designCandidateRow.click()
  await expect(page.getByTestId('llm-run-viewer')).toBeVisible()
  await expect(page.getByTestId('llm-run-viewer')).toContainText('実行時刻')
  ...
  // 詳細画面からも同じ応答を再構成して開ける（LLMは再実行しない）
  await page.getByTestId('llm-open-candidate').click()
  await expect(page.getByTestId('candidate-editor')).toBeVisible()
  await page.keyboard.press('Control+W')
  await page.getByTestId('status-jobs').click()
  await page.getByTestId('panel-tab-llm').click()
  ...
  // ログからの候補再作成（LLM再実行）も、送信内容確認と明示承認を経てジョブ登録する
...
test('MCPサーバの設定・起動・応答・停止（MCP-001〜008）', async () => {
  // 前回実行の永続設定（userData の settings.json）が残っても開始条件を停止状態に固定する
  await page.evaluate(async () => window.api.invoke('mcp.applySettings', { enabled: false }))

  // Status Bar の MCP 表示からツール設定を開く（MCP-003）
  const statusMcp = page.getByTestId('status-mcp')
  await expect(statusMcp).toContainText('MCP: 停止')
  await statusMcp.click()
  const settings = page.locator('[data-testid="settings-editor"]:visible')
  await expect(settings).toHaveCount(1)
  const section = settings.getByTestId('setting-mcp-section')
  await section.scrollIntoViewIfNeeded()
  await expect(section).toContainText('MCPサーバ設定')

  // 有効化＋ポート指定→適用でローカルサーバが起動する（MCP-001/002）
  const mcpPort = 39455
  await section.getByTestId('setting-mcp-enabled').check()
  await section.getByTestId('setting-mcp-port').fill(String(mcpPort))
  await section.getByTestId('setting-mcp-apply').click()
  await expect(page.getByTestId('notifications')).toContainText('MCPサーバを起動しました')
  await expect(section.getByTestId('setting-mcp-status')).toContainText('起動中')
  await expect(statusMcp).toContainText(`MCP: 起動中 :${mcpPort}`)

  // テストプロセスから Streamable HTTP（JSON-RPC 2.0）で実応答を確認（MCP-004〜008 のツール公開）
  const rpc = async (payload: unknown): Promise<unknown> => {
  ...
  expect(initialize.result.serverInfo.name).toBe('d2d-design-info')
  ...
  const listTypes = (await rpc({
    ...
    id: 3,
    method: 'tools/call',
    params: { name: 'list_element_types', arguments: {} }
  })) as { result: { isError?: boolean; content: { text: string }[] } }
  expect(listTypes.result.isError).toBeUndefined()
  expect(JSON.parse(listTypes.result.content[0]!.text).design_models.length).toBeGreaterThan(0)

  // 無効化→適用で停止し、Status Bar も停止表示へ戻る（MCP-003）
  await section.getByTestId('setting-mcp-enabled').uncheck()
  await section.getByTestId('setting-mcp-apply').click()
  await expect(page.getByTestId('notifications')).toContainText('MCPサーバを停止しました')
  await expect(section.getByTestId('setting-mcp-status')).toContainText('停止')
  await expect(statusMcp).toContainText('MCP: 停止')
})
...
test('PDF原本→領域候補生成→オーバーレイレビュー編集・保存（P5-20A/B）', async () => {
  ...
  // LLM分類支援の送信前確認: 選択領域と近傍ブロックだけを送る（P5-20D、検討資料§16.3）
  const llmTargetRow = page
    .getByTestId('pdf-region-list')
    .locator('.excel-candidate-row', { hasText: '見出し' })
    .first()
  await llmTargetRow.locator('button').click()
  await page.getByTestId('pdf-region-llm').click()
  await expect(page.getByTestId('llm-request-dialog')).toBeVisible()
  await page.getByTestId('llm-preview-button').click()
  await expect(page.getByTestId('llm-preview')).toContainText('System Configuration')
  await page.getByTestId('llm-request-dialog').getByRole('button', { name: '閉じる', exact: true }).click()

  // Vision OCRの送信前確認: 領域切出し画像を添付する（P5-20D、EXT-030）
  await page.getByTestId('pdf-ocr-prepare').click()
  await expect(page.getByTestId('llm-request-dialog')).toBeVisible({ timeout: 30_000 })
  await page.getByTestId('llm-request-dialog').getByRole('button', { name: '閉じる', exact: true }).click()

  // 確定: 採用領域だけを②抽出データへ変換し、ページ・座標トレースを保持する（P5-20C、EXT-031）
  await page.getByTestId('pdf-region-confirm').click()
  await expect(page.getByTestId('extraction-review-editor')).toBeVisible({ timeout: 30_000 })
  await expect(page.getByTestId('element-grid')).toContainText('1. System Configuration')
  await expect(page.getByTestId('element-grid')).toContainText('Table 1 Component list')
  await expect(page.getByTestId('element-grid')).not.toContainText('D2D SPEC')
  await expect(page.getByTestId('element-grid')).not.toContainText('This system consists')
  const documentAfter = await page.evaluate(
    async (uid) => window.api.invoke<{ has_extracted_data: number }>('document.get', { uid }),
    sourceRow!.uid
  )
  expect(documentAfter).toMatchObject({ ok: true, result: { has_extracted_data: 1 } })
  // 構造データに表セル・ページ座標トレースが保持される（EXT-027/029/031）
  const structure = await page.evaluate(async (fileName) => {
    const list = await window.api.invoke<Array<{ uid: string; title: string }>>('extracted.list', {})
    if (!list.ok) return null
    const target = list.result.find((item) => item.title === fileName)
    if (!target) return null
    const detail = await window.api.invoke<{ elements: unknown[] }>('extracted.get', { uid: target.uid })
    return detail.ok ? JSON.stringify(detail.result.elements) : null
  }, basename(pdfPath))
  expect(structure).not.toBeNull()
  expect(structure!).toContain('Controller')
  expect(structure!).toContain('"page_no_start"')
  expect(structure!).toContain('"bbox"')
  expect(structure!).not.toContain('D2D SPEC')

  // 確定後も stage://source から候補を読取専用で再表示できる
  await page.getByTestId('resource-address').fill('stage://source')
  await page.getByTestId('resource-address').press('Enter')
  await page.getByTestId(`stage-source-row-${sourceRow!.code}`).click()
  await expect(page.getByTestId('source-stage-preview').getByTestId('extract-button')).toBeDisabled()
  await expect(page.getByTestId('source-stage-preview').getByTestId('pdf-review-button')).toBeEnabled()
  await page.getByTestId('source-stage-preview').getByTestId('pdf-review-button').click()
  await expect(page.getByTestId('pdf-extraction-editor')).toBeVisible()
  await expect(page.getByTestId('pdf-region-confirm')).toBeDisabled()
  await expect(page.getByTestId('pdf-region-add')).toBeDisabled()
